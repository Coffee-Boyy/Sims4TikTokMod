...
def _hash_add(cls, fields):
    flds = [f for f in fields if f.hash is None]
...
def make_dataclass(cls_name, fields, *, bases=(), namespace=None, init=True, repr=True, eq=True, order=False, unsafe_hash=False, frozen=False):
    ...
    for item in fields:
        if isinstance(item, str):
            ...
            raise TypeError(f'Invalid field: {item}')
        if not (isinstance(name, str) and name.isidentifier()):
            raise TypeError(f'Field names must be valid identifers: {name}')
        if keyword.iskeyword(name):
            raise TypeError(f'Field names must not be keywords: {name}')
        if name in seen:
            raise TypeError(f'Field name duplicated: {name}')
        seen.add(name)
        anns[name] = tp
