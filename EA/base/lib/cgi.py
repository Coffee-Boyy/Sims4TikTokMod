...
class FieldStorage:
    ...
    def __init__(self, fp=None, headers=None, outerboundary=b'', environ=os.environ, keep_blank_values=0, strict_parsing=0, limit=None, encoding='utf-8', errors='replace'):
        ...
        self.done = 0
        if self.limit is None and ctype == 'application/x-www-form-urlencoded':
    ...
    def __write(self, line):
        ...
        if self._FieldStorage__file is not None and self._binary_file:
    ...
    def skip_lines(self):
        if self.outerboundary and self.done:
            return
        next_boundary = b'--' + self.outerboundary
        last_boundary = next_boundary + b'--'
        last_line_lfend = True
