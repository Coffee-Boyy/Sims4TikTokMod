...
def _type_check(arg, msg, is_argument=True):
    ...
    if isinstance(arg, _SpecialForm) and arg is not Any or arg in (Generic, _Protocol):
...
class ForwardRef(_Final, _root=True):
    __slots__ = ('__forward_arg__', '__forward_code__', '__forward_evaluated__', '__forward_value__', '__forward_is_argument__')
    ...
    def _evaluate(self, globalns, localns):
        if self.__forward_evaluated__ and localns is not globalns:
            if globalns is None and localns is None:
                ...
                localns = globalns
            self.__forward_value__ = _type_check(eval(self.__forward_code__, globalns, localns), 'Forward references must evaluate to types.', is_argument=self.__forward_is_argument__)
...
_normalize_alias = {'list': 'List', 'tuple': 'Tuple', 'dict': 'Dict', 'set': 'Set', 'frozenset': 'FrozenSet', 'deque': 'Deque', 'defaultdict': 'DefaultDict', 'type': 'Type', 'Set': 'AbstractSet'}
def _is_dunder(attr):
...
class _GenericAlias(_Final, _root=True):
    ...
    def __init__(self, origin, params, *, inst=True, special=False, name=None):
        ...
        self._name = name
        if not (special and isinstance(params, tuple)):
            params = (params,)
    ...
    def __new__(cls, typename, bases, ns):
        ...
        for field_name in types:
            if field_name in ns:
                ...
                raise TypeError('Non-default namedtuple field {field_name} cannot follow default field(s) {default_names}'.format(field_name=field_name, default_names=', '.join(defaults_dict.keys())))
