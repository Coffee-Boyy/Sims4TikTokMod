...
class Test_TestCase(unittest.TestCase, TestEquality, TestHashing):
    ...
    def testAssertRaisesContext(self):
        ...
        with self.assertRaises(ValueError):
            int('19', base=8)
        with self.assertRaises(self.failureException), self.assertRaises(ExceptionMock):
            pass
        with self.assertRaisesRegex(self.failureException, 'foobar'), self.assertRaises(ExceptionMock, msg='foobar'):
            pass
        with self.assertWarnsRegex(DeprecationWarning, 'foobar'), self.assertRaises(AssertionError), self.assertRaises(ExceptionMock, foobar=42):
            pass
        with self.assertRaises(ExceptionMock):
            self.assertRaises(ValueError, Stub)
    ...
    def testAssertNotRaisesRegex(self):
        ...
        with self.assertRaisesRegex(self.failureException, 'foobar'), self.assertRaisesRegex(Exception, 'expect', msg='foobar'):
            pass
        with self.assertWarnsRegex(DeprecationWarning, 'foobar'), self.assertRaises(AssertionError), self.assertRaisesRegex(Exception, 'expect', foobar=42):
            pass
    ...
    def testAssertWarnsContext(self):
        ...
        with self.assertRaises(self.failureException), self.assertWarns(RuntimeWarning):
            pass
        with self.assertRaisesRegex(self.failureException, 'foobar'), self.assertWarns(RuntimeWarning, msg='foobar'):
            pass
        with self.assertWarnsRegex(DeprecationWarning, 'foobar'), self.assertRaises(AssertionError), self.assertWarns(RuntimeWarning, foobar=42):
            pass
        with warnings.catch_warnings():
            warnings.simplefilter('default', RuntimeWarning)
            with self.assertRaises(self.failureException), self.assertWarns(DeprecationWarning):
                _runtime_warn()
        with warnings.catch_warnings():
            warnings.simplefilter('error', RuntimeWarning)
            with self.assertRaises(RuntimeWarning), self.assertWarns(DeprecationWarning):
                _runtime_warn()
    ...
    def testAssertWarnsRegexContext(self):
        ...
        self.assertIn('test_case.py', cm.filename)
        self.assertEqual(cm.lineno, _runtime_warn_lineno + 1)
        with self.assertRaises(self.failureException), self.assertWarnsRegex(RuntimeWarning, 'o+'):
            pass
        with self.assertRaisesRegex(self.failureException, 'foobar'), self.assertWarnsRegex(RuntimeWarning, 'o+', msg='foobar'):
            pass
        with self.assertWarnsRegex(DeprecationWarning, 'foobar'), self.assertRaises(AssertionError), self.assertWarnsRegex(RuntimeWarning, 'o+', foobar=42):
            pass
        with warnings.catch_warnings():
            warnings.simplefilter('default', RuntimeWarning)
            with self.assertRaises(self.failureException), self.assertWarnsRegex(DeprecationWarning, 'o+'):
                _runtime_warn('foox')
        with self.assertRaises(self.failureException), self.assertWarnsRegex(RuntimeWarning, 'o+'):
            _runtime_warn('barz')
        with warnings.catch_warnings():
            warnings.simplefilter('error', RuntimeWarning)
            with self.assertRaises((RuntimeWarning, self.failureException)), self.assertWarnsRegex(RuntimeWarning, 'o+'):
                _runtime_warn('barz')
    ...
    def testAssertLogsFailureNoLogs(self):
        with self.assertNoStderr(), self.assertRaises(self.failureException), self.assertLogs():
            pass

    def testAssertLogsFailureLevelTooHigh(self):
        with self.assertNoStderr(), self.assertRaises(self.failureException), self.assertLogs(level='WARNING'):
            log_foo.info('1')

    def testAssertLogsFailureMismatchingLogger(self):
        with self.assertLogs('quux', level='ERROR'), self.assertRaises(self.failureException), self.assertLogs('foo'):
            log_quux.error('1')
