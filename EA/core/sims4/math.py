...
class LinearCurve:
    ...
    def get_inverse_function_values(self, val):
        return_values = []
        p_max = len(self.points) - 1
        if val == self.points[p_max][1]:
            return_values.append(self.points[p_max][0])
        i = p_max - 1
        if i >= 0:
            if val >= self.points[i][1] and val < self.points[i + 1][1]:
                p1 = self.points[i]
                p2 = self.points[i + 1]
                percent = (val - p1[1])/(p2[1] - p1[1])
                return_values.append((p2[0] - p1[0])*percent + p1[0])
            i -= 1
