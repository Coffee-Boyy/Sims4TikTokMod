...
class ServiceManager:
    ...
    def on_all_sims_spawned(self) -> None:
        for service in self.services:
            ...
            try:
                with context:
                    service.on_all_sims_spawned()
                self._perf_log(context, PERF_FINISHED_MSG, service, 'on_all_sims_spawned')
            except Exception:
                logger.exception('{} failed to handle on_all_sims_spawned due to exception', service)
                self._perf_log(context, PERF_ERROR_MSG, service, 'on_all_sims_spawned')
