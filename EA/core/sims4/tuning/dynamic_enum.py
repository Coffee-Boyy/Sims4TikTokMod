...
def _get_pack_from_enum_value(enum_value):
    ...
    return Pack((int(enum_value) - 8192)//2048 + 1)
...
class DynamicEnumMetaclass(enum.Metaclass):
    ...
    def is_enum_value_available(cls, value):
        if not cls.partitioned:
            return True
        return is_available_pack(_get_pack_from_enum_value(value))
