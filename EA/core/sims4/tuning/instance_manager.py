...
class TuningInstanceManager(Service):
    ...
    def __init__(self, instance_manager_list:'List[InstanceManager]', registered_callbacks_timing_file:'Optional[TextIO]'=None, packs_to_load:'Optional[Tuple[Pack]]'=None, increment_completed_callback:'Optional[Callable[[], None]]'=None) -> 'None':
        ...
        self.packs_to_load = packs_to_load
        self.increment_completed_callback = increment_completed_callback
    ...
    def update_incremental_start(self):
        ...
        if self.increment_completed_callback is not None:
            self.increment_completed_callback()
        return True
    ...
    def _execute_gen(self, log_fn=None):
        ...
        pending_reload_dict = defaultdict(set) if log_fn is None and self.packs_to_load is not None else None
        for instance_manager in self._instance_managers:
            self._execute_func_and_calculate_time(instance_manager, instance_manager.create_class_instances, self.packs_to_load, pending_reload_dict)
            yield False
        for instance_manager in self._instance_managers:
            inst_ids = pending_reload_dict.get(instance_manager.TYPE)
            if inst_ids is not None:
                instance_manager.pending_reload_inst_ids.update(inst_ids)
        log_fn(LOADING_INSTANCES)
        for instance_manager in self._instance_managers:
            self._execute_func_and_calculate_time(instance_manager, instance_manager.load_data_into_class_instances, self.packs_to_load)
            yield False
        log_fn(INVOKING_CALLBACKS)
        for instance_manager in self._instance_managers:
            if calculate_stats_flag:
                time_stamp = time.time()
                yield from instance_manager.invoke_registered_callbacks_gen(registered_callbacks_timing_file=self.registered_callbacks_timing_file, packs_to_load=self.packs_to_load)
                instance_manager.load_time += time.time() - time_stamp
            else:
                yield from instance_manager.invoke_registered_callbacks_gen(registered_callbacks_timing_file=self.registered_callbacks_timing_file, packs_to_load=self.packs_to_load)
        log_fn(INVOKING_ON_PACK_LOADED)
        for instance_manager in self._instance_managers:
            self._execute_func_and_calculate_time(instance_manager, instance_manager.on_packs_loaded)
    ...
    def _execute_func_and_calculate_time(self, instance_manager:'InstanceManager', execute_function:'Callable', *args, **kwargs) -> 'None':
        if calculate_stats_flag:
            time_stamp = time.time()
    ...
    def unload_packs(self, packs_to_unload:'Tuple[Pack]') -> 'None':
        pending_reload_dict = defaultdict(set)
        for instance_manager in self._instance_managers:
            instance_manager.unload_packs(packs_to_unload, pending_reload_dict)
        for instance_manager in self._instance_managers:
            inst_ids = pending_reload_dict.get(instance_manager.TYPE)
            if inst_ids is not None:
                instance_manager.pending_reload_inst_ids.update(inst_ids)
        for instance_manager in self._instance_managers:
            instance_manager.on_packs_loaded()
...
class InstanceManager:
    _tuning_dependencies_cache = None
    ...
    def __init__(self, path, type_enum, use_guid_for_ref=False, base_game_only=False, require_reference=False):
        ...
        self.pending_reload_inst_ids = set()
    ...
    def reload_dependency_tunings(self) -> 'None':
        for inst_id in self.pending_reload_inst_ids:
            self.reload_by_instance_id(inst_id)
        self.pending_reload_inst_ids.clear()

    def reload_by_instance_id(self, instance_id:'int', local_res_key:'Optional[sims4.resources.Key]'=None) -> 'bool':
        registered_resource_key = sims4.resources.Key(self.TYPE, instance_id)
        cls = self._tuned_classes.get(registered_resource_key)
        if cls is None:
            return False
        else:
            key = local_res_key if local_res_key is not None else registered_resource_key
            try:
    ...
    def reload_by_key(self, key:'sims4.resources.Key') -> 'Optional[Set[sims4.resources.Key]]':
        raise RuntimeError('[manus] Reloading tuning is not supported for optimized python builds.')
        if self.reload_by_instance_id(key.instance, local_res_key=key):
    ...
    def get_ordered_types(self, only_subclasses_of=None):
        ...
        def key(cls):
            if self.is_instanced_class_tuning_manager:
                cls = type(cls)
        ...
        if only_subclasses_of is None:
            ...
            check_subclass_func = isinstance if self.is_instanced_class_tuning_manager else issubclass
            result = [c for c in self.types.values() if check_subclass_func(c, only_subclasses_of)]
    ...
    def on_packs_loaded(self) -> 'None':
        self.reload_dependency_tunings()
    ...
    def create_class_instances(self, packs_to_load:'Optional[Tuple[Pack]]'=None, pending_reload_dict:'Optional[DefaultDict[int, Set[int]]]'=None) -> 'None':
        mtg = get_manager()
        ...
        if packs_to_load is None:
            ...
            if InstanceManager._tuning_dependencies_cache is None:
                InstanceManager._tuning_dependencies_cache = read_td_cache_from_resource()
            for (group_id, instance_id) in res_id_list:
                if group_id not in packs_to_load:
                    pass
                else:
                    res_key = sims4.resources.Key(self.TYPE, instance_id, group_id)
                    if pending_reload_dict is not None:
                        reference_key = (instance_id, self.TYPE)
                        referrer_id_type_pairs = InstanceManager._tuning_dependencies_cache.get(reference_key)
                        if referrer_id_type_pairs is not None:
                            for (inst_id, inst_type) in referrer_id_type_pairs:
                                pending_reload_dict[inst_type].add(inst_id)
                            logger.info('Tuning dependency found: {} referenced by {}', reference_key, referrer_id_type_pairs, owner='yozhang')
                    self._create_class_instance(res_key, group_id)
    ...
    def load_data_into_class_instances(self, packs_to_load:'Optional[Tuple[Pack]]'=None):
        ...
        for (key, cls) in tuple(self._tuned_classes.items()):
            try:
                ...
                if key.group not in packs_to_load:
                    continue
    ...
    def invoke_registered_callbacks_gen(self, registered_callbacks_timing_file:'Optional[TextIO]'=None, packs_to_load:'Optional[Tuple[Pack]]'=None):
        ...
        for (key, cls) in self._tuned_classes.items():
            if packs_to_load is not None:
                group_id = mtg.res_id_group_map.get(key.instance)
    ...
    def invoke_verify_tuning_callback_gen(self):
        for cls in self._tuned_classes.values():
            ...
            yield False
        self._verify_tunable_callback_helper.clear()
    ...
    @classmethod
    def verify_class_restrictions(cls, class_restrictions, value, source=None):
        if class_restrictions and value is not None:
            value_mro_set = set([c.__name__ for c in value.mro()])
            res_cls = type(value) if cls.is_instanced_class_tuning_manager else value
            for c in class_restrictions:
                if isinstance(c, str):
                    if c in value_mro_set:
                        break
                        if issubclass(res_cls, c):
                            break
                elif issubclass(res_cls, c):
                    break
            raise ValueError('TunableReference in {} is set to a value that is not allowed by its class restriction.'.format(source))

    def unload_packs(self, packs_to_unload:'Tuple[Pack]', pending_reload_dict:'DefaultDict[int, Set[int]]') -> 'None':
        for (key, cls) in tuple(self._tuned_classes.items()):
            remapped_key = self.remapped_keys.get(key)
            if remapped_key is not None and remapped_key.group in packs_to_unload:
                del self._tuned_classes[key]
                if InstanceManager._tuning_dependencies_cache is None:
                    InstanceManager._tuning_dependencies_cache = read_td_cache_from_resource()
                reference_key = (key.instance, self.TYPE)
                referrer_id_type_pairs = InstanceManager._tuning_dependencies_cache.get(reference_key)
                if referrer_id_type_pairs is not None:
                    for (inst_id, inst_type) in referrer_id_type_pairs:
                        pending_reload_dict[inst_type].add(inst_id)
    ...
    def get(self, name_or_id_or_key, pack_safe=False, get_fallback_definition_id=True, class_restrictions=(), source=None):
        key = sims4.resources.get_resource_key(name_or_id_or_key, self.TYPE)
        cls = self._tuned_classes.get(key)
        if cls is None:
            if not pack_safe:
                return
            add_tuning_dependency(source, key.instance, key.type)
            raise sims4.tuning.merged_tuning_manager.UnavailablePackSafeResourceError
        self.verify_class_restrictions(class_restrictions, cls, source)
        return cls

    def _instantiate(self, target_type):
        return target_type()

    def get_descriptions_to_export(self, export_path, filter_fn=None):
        ...
        return [(cls, export_path, self.TYPE) for cls in to_export.values()]
