...
class MergedTuningManager:
    ...
    def __init__(self):
        ...
        self._skip_packs = None
        parser = argparse.ArgumentParser()
        parser.add_argument('--skip_packs', default='', help='Calculate Tuning Data and Store in csv files ')
        (args, unused_args) = parser.parse_known_args()
        if args.skip_packs:
            self._skip_packs = set()
            packs = args.skip_packs.split(',')
            for pack_folder_name in packs:
                self._skip_packs.add(get_pack_enum(pack_folder_name))
            set_unloaded_packs(self._skip_packs)

    def load(self, silent_fail=True):
        if self._skip_packs is None:
            ...
            for combined_tuning_key in get_all_resources_of_type(type_id=sims4.resources.Types.COMBINED_TUNING):
                if combined_tuning_key.group not in self._skip_packs:
                    self._load_combined_file_by_key(combined_tuning_key, silent_fail=silent_fail)

    def prepare_hot_load_unload(self, packs:'Tuple[Pack]') -> 'bool':
        keys = get_all_resources_of_type(type_id=sims4.resources.Types.COMBINED_TUNING)
        packs_have_tuning = set(k.group for k in keys) & set(packs)
        if not packs_have_tuning:
            return False
        for combined_tuning_key in keys:
            self._load_combined_file_by_key(combined_tuning_key, silent_fail=True)
        return True
    ...
    def _load_res_node(self, res_node, group_id):
        ...
        for child_node in res_node:
            ...
            if group_id != 0:
                self.res_id_group_map[res_id] = group_id
