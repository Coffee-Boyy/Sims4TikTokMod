...
class Pack(enum.Int):
    ...
    def _is_available_pack(pack):
        return pack in available_packs
_is_available_pack = _zone.is_available_packavailable_packs = {pack for pack in Pack if _is_available_pack(pack)}with sims4.reload.protected(globals()):
    unloaded_packs = set()
class UnavailablePackError(ValueError):
    pass

def set_unloaded_packs(packs:'Set[Pack]') -> 'None':
    unloaded_packs.update(packs)

def remove_unloaded_packs(packs:'Set[Pack]') -> 'None':
    global unloaded_packs
    unloaded_packs = unloaded_packs.difference(packs)

def is_available_pack(pack:'Pack') -> 'bool':
    if pack in unloaded_packs:
        return False
    return _is_available_pack(pack)
