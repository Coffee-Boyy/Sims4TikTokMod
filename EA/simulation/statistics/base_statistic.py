...
class BaseStatistic(HasTunableLodMixin):
    ...
    def set_value(self, value:float, from_load:bool=False, **kwargs) -> None:
        ...
        self._value = value
        self._clamp()
        self._notify_change(old_value, from_load=from_load)

    def _notify_change(self, old_value:float, from_load:bool=False) -> None:
        ...
        if self.telemetry:
            ...
            if not owner.is_sim:
                sim_info = owner
            with telemetry_helper.begin_hook(writer, TELEMETRY_HOOK_CHANGE, False, None, sim_info) as hook:
                hook.write_float('sage', sim_info.age if sim_info else 0)
                hook.write_guid('guid', self.guid64)
                hook.write_float('sadd', value - old_value)
                hook.write_float('sttl', value)
                hook.write_int('rank', self.rank_level if self.is_ranked else 0)
