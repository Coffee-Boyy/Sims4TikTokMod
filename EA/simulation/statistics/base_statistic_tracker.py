...
class BaseStatisticTracker(SimInfoTracker):
    ...
    def set_value(self, stat_type, value, add=DEFAULT, from_load=False, from_init=False, from_transfer=False, **kwargs):
        if add is DEFAULT:
        ...
        if from_init and add and stat is not None:
            stat.set_value(value, from_load=from_load, from_transfer=from_transfer)
    ...
    def debug_output_all(self, _connection):
        if self._statistics:
            for (stat_type, stat) in self._statistics.items():
                sims4.commands.output('{:<24} Value: {:-6.2f}'.format(stat_type.__name__, stat.get_value()) if stat else '{:<24} Value: None'.format(stat_type.__name__), _connection)

    def debug_output_all_automation(self, _connection):
        if self._statistics:
            for (stat_type, stat) in self._statistics.items():
                sims4.commands.automation_output('Statistic; Status:Data, Name:{}, Value:{}'.format(stat_type.__name__, stat.get_value() if stat else 'None'), _connection)
