...
class Broadcaster(TuningClassMixin, RouteEventProviderMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.BROADCASTER)):
    ...
    def __init__(self, *args, broadcasting_object, interaction:'Interaction'=None, excluded_participant_cache:'Dict'=None, init_blueprint_func:'Callable'=None, **kwargs):
        ...
        if self.register_state_changed:
            broadcasting_object.state_component.add_state_changed_callback(self.on_broadcaster_state_changed)
        self._quadtree = None
    ...
    def on_removed(self):
        ...
        if broadcasting_object is not None:
            ...
            if self.register_state_changed:
                broadcasting_object.state_component.remove_state_changed_callback(self.on_broadcaster_state_changed)
    ...
    def on_broadcaster_state_changed(self, owner, state:'ObjectState', old_value:'ObjectStateValue', new_value:'ObjectStateValue'):
        if self._affected_objects is None:
            return
        for affected_object in self._affected_objects.keys():
            for broadcaster_effect in self.effects:
                broadcaster_effect.on_broadcaster_state_changed(self, state, old_value, new_value, affected_object)
