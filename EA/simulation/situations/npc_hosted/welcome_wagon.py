...
class PreWelcomeWagon(SituationComplexCommon):
    ...
    def should_seed_be_loaded(cls, seed, load_open_street_situation_with_selectable_sim=False):
        if not seed.guest_list.get_guest_infos_for_job(cls._door_knocker_situation_job):
            return False
        return super().should_seed_be_loaded(seed, load_open_street_situation_with_selectable_sim)
    ...
    def create_welcome_wagon(self):
        ...
        if not situation_manager.is_incompatible_user_facing_situation_running():
