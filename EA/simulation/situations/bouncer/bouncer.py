...
class Bouncer(sims.sim_spawner_service.ISimSpawnerServiceCustomer):
    ...
    def _assign_instanced_sims_to_unfulfilled_requests(self) -> 'None':
        with situations.situation_manager.DelayedSituationDestruction():
                    ...
                    if not request._requires_spawning:
                        if request._status != BouncerRequestStatus.SUBMITTED:
                            ...
                            if not candidate_sim_ids:
                                ...
                                def get_filter_results(requesting_sim_info:'SimInfo') -> 'List':
                                    return sim_filter_service.submit_filter(request._sim_filter, callback=None, sim_constraints=list(candidate_sim_ids), blacklist_sim_ids=blacklist, requesting_sim_info=requesting_sim_info, allow_yielding=False, additional_filter_terms=request.get_additional_filter_terms(), gsi_source_fn=self.get_sim_filter_gsi_name)

                                filter_results = set()
                                filter_results.update(get_filter_results(request._requesting_sim_info))
                                additional_sim_infos = request.situation.additional_requesting_sim_infos.get(request.job_type, [])
                                for sim_info in additional_sim_infos:
                                    filter_results.update(get_filter_results(sim_info))
