...
class RepoSituation(SituationComplexCommon):
    ...
    def reduce_debt(self, amount):
        ...
        elif self.debt_source == DebtSource.BILLS or self.debt_source == DebtSource.HOUSING_BILLS:
            services.active_household().bills_manager.reduce_amount_owed(int(amount), self.debt_source)
        else:
    ...
    def clear_debt(self):
        if self.debt_source == DebtSource.SCHOOL_LOAN:
            ...
            rental_unit_manager = services.business_service().get_business_manager_for_zone(services.active_household().home_zone_id)
            if rental_unit_manager is not None and rental_unit_manager.business_type == BusinessType.RENTAL_UNIT:
                services.active_household().bills_manager.pay_non_housing_bill()
            else:
    ...
    def get_debt_value(self):
        ...
        if self.debt_source == DebtSource.BILLS:
            rental_unit_manager = services.business_service().get_business_manager_for_zone(services.active_household().home_zone_id)
            if rental_unit_manager is not None and rental_unit_manager.business_type == BusinessType.RENTAL_UNIT:
                return services.active_household().bills_manager.non_housing_costs_owed
            return services.active_household().bills_manager.current_payment_owed
        elif self.debt_source == DebtSource.HOUSING_BILLS:
            return services.active_household().bills_manager.housing_costs_owed
        else:
            logger.error('Attempting to use a debt source that is not handled', owner='jjacobson')
            return
