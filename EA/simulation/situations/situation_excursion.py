...
class ExcursionSituation(BaseCustomStatesSituation):
    ...
    def _get_activity_duration(self) -> 'TimeSpan':
        duration = self.current_activity.duration + random.randint(0, self.current_activity.duration_randomizer)
        return create_time_span(minutes=duration)
    ...
    def _on_activity_changed(self):
        ...
        self.change_duration_by_timespan(new_activity_duration)
        self.send_activity_telemetry('start')
