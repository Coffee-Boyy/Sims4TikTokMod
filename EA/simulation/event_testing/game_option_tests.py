...
class TestableGameOptions(enum.IntFlags):
    ...
    BURGLAR_ENABLED = ...
    RESTRICT_NPC_FAIRIES = ...
    BALANCE_SYSTEM_ENABLED = ...
    LUCK_ENABLED = ...
    AILMENTS_ENABLED = ...
    REQUIRED_PACK_BY_OPTION = frozendict({AILMENTS_ENABLED: Pack.EP19, LUCK_ENABLED: Pack.EP19, BALANCE_SYSTEM_ENABLED: Pack.EP19, RESTRICT_NPC_FAIRIES: Pack.EP19, HEIRLOOM_OBJECTS_ENABLED: Pack.EP17, CAREER_LAY_OFF_ENABLED: Pack.EP13, SELF_DISCOVERY_ENABLED: Pack.EP13, RESTRICT_NPC_WEREWOLVES: Pack.GP12, DUST_SYSTEM_ENABLED: Pack.SP22, ECO_FOOTPRINT_GAMEPLAY: Pack.EP09, CIVIC_POLICY_NPC_VOTING_ENABLED: Pack.EP09})
...
class SimInfoGameplayOptionsTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def __call__(self, test_targets=None):
        ...
        elif self.gameplay_option == TestableGameOptions.RESTRICT_NPC_FAIRIES:
            misc_option_service = services.misc_options_service()
            if misc_option_service is not None:
                value = misc_option_service.restrict_npc_fairies
        ...
        elif self.gameplay_option == TestableGameOptions.BALANCE_SYSTEM_ENABLED:
            misc_option_service = services.misc_options_service()
            if misc_option_service is not None:
                value = misc_option_service.balance_system_enabled
        elif self.gameplay_option == TestableGameOptions.LUCK_ENABLED:
            luck_service = services.get_luck_service()
            if luck_service is not None:
                value = luck_service.luck_enabled
        elif self.gameplay_option == TestableGameOptions.AILMENTS_ENABLED:
            misc_option_service = services.misc_options_service()
            if misc_option_service is not None:
                value = misc_option_service.ailments_enabled
        if value is None:
