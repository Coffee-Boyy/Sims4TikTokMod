...
class _SimoleonTestValueContextPropertyOnly(_SimoleonTestValueContextBase):
    FACTORY_TUNABLES = {'allow_inactive_lot_value': Tunable(description="\n            Whether or not to allow the value of the household to be determined when it doesn't belong to the active\n            lot's owner. For example, if this is enabled, the household value for the player's Sim will be evaluated\n            while at the park instead of being 0.\n            ", tunable_type=bool, default=False)}

    def _get_value(self, subject):
        household = services.household_manager().get_by_sim_id(subject.sim_id)
        return household.get_property_value(self.allow_inactive_lot_value)
            ...
            for career in careers_to_check:
                ...
                if hours is None:
                    ...
                    if time_span is None:
                        return TestResultNumeric(False, '{0} does not currently have any hours scheduled in career {1}', subject, career, current_value=-1, goal_value=self.hours_till_work.lower_bound, is_money=False, tooltip=tooltip)
                    hours = time_span.in_hours()
                if self.hours_till_work.lower_bound <= hours and hours <= self.hours_till_work.upper_bound:
                    pass
                else:
                    return TestResultNumeric(False, '{0} does not currently have the correct hours till work in career ({1},{2}) required to pass this test', subject, career, self.hours_till_work, current_value=hours, goal_value=self.hours_till_work.lower_bound, is_money=False, tooltip=tooltip)
        return TestResult.TRUE

class CareerPTOAmountTestFactory(SpecifiedCareerMixin, HasTunableSingletonFactory, AutoFactoryInit):
    UNIQUE_TARGET_TRACKING_AVAILABLE = True
    FACTORY_TUNABLES = {'career': TunableReference(description='\n            The career to test for how much PTO the sim has.\n          \n            If None, will expect career passed in via PickedItemIds (i.e. via picker).\n            ', manager=services.get_instance_manager(sims4.resources.Types.CAREER), allow_none=True), 'required_pto_available': TunableThreshold(description='\n            Threshold test for how much PTO is required\n            ')}
    ...
    def do_test(self, subjects, career_id, tooltip):
        for subject in subjects:
            ...
            if self.required_pto_available.compare(pto_available):
                ...
                return TestResultNumeric(False, '{0} does not currently have the correct amount of PTO in career ({1},{2}) required to pass this test', subject, self.career, self.required_pto_available, current_value=pto_available, goal_value=self.required_pto_available.value, is_money=False, tooltip=tooltip)
        return TestResult.TRUE

class CareerHasIconOverrideTest(SpecifiedCareerMixin, HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'career': TunableReference(description='\n            The career to check whether it has an icon override on the subject. \n            ', manager=services.get_instance_manager(sims4.resources.Types.CAREER), pack_safe=True)}
...
class CareerHasAssignmentTestFactory(SpecifiedCareerMixin, HasTunableSingletonFactory, AutoFactoryInit):
    UNIQUE_TARGET_TRACKING_AVAILABLE = True
    FACTORY_TUNABLES = {'career': TunableReference(description='\n            The career to test for having an available assignment.\n           \n            If None, will expect career passed in via PickedItemIds (i.e. via picker).\n            ', manager=services.get_instance_manager(sims4.resources.Types.CAREER), allow_none=True)}
...
class CareerReferenceTestFactory(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    @staticmethod
    def _verify_tunable_callback(instance_class, tunable_name, source, value):
        if value.career is not None and len(value.career_list) > 0:
    ...
    def __call__(self, subjects, tooltip=None, **kwargs):
        ...
        for subject in subjects:
            ...
            if not subject_careers:
                return TestResult(False, '{0} does not currently have any careers and a career is needed for this test: {1}/{2}:{3}.', subject, self.career, self.career_list, self.user_level, tooltip=tooltip)
            ...
            if current_user_level:
                return TestResultNumeric(False, '{0} does not currently have the correct career/user level ({1},{2}) required to pass this test.', subject, self.career, self.user_level, current_value=current_user_level, goal_value=self.user_level.lower_bound, is_money=False, tooltip=tooltip)
...
class CareerTrackTestFactory(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def __call__(self, subjects, targets=None, tooltip=None):
        ...
        for subject in subjects:
            for career in subject.careers.values():
                if career.current_track_tuning == self.career_track and self.user_level and career.user_level >= self.user_level.lower_bound:
...
class CareerEventTest(HasTunableSingletonFactory, AutoFactoryInit):
    UNIQUE_TARGET_TRACKING_AVAILABLE = False
    FACTORY_TUNABLES = {'career_event': OptionalTunable(description='\n            The career event to test for.\n            ', tunable=TunableReference(manager=services.get_instance_manager(sims4.resources.Types.CAREER_EVENT)), disabled_name='any', enabled_name='specific_CareerEvent')}
        ...
        for subject in subjects:
            ...
            for career in sim_info.career_tracker:
                ...
                if career_event_manager is None:
                    ...
                    if not self.career_event is None:
                        if career_event_manager.is_top_career_event(self.career_event):
                            return TestResult.TRUE
                    return TestResult.TRUE
...
class TunableCareerTestVariant(TunableVariant):
    ...
    def __init__(self, **kwargs):
        tunables = {'attended_first_day': CareerAttendedFirstDay.TunableFactory(), 'days_worked': CareerDaysWorked.TunableFactory(), 'has_career': HasCareerTestFactory.TunableFactory(), 'has_quittable_career': QuittableCareerTestFactory.TunableFactory(), 'has_available_assignment': CareerHasAssignmentTestFactory.TunableFactory(), 'can_join_career': CareerAvailabilityTestFactory.TunableFactory(), 'career_reference': CareerReferenceTestFactory.TunableFactory(), 'career_track': CareerTrackTestFactory.TunableFactory(), 'career_level': CareerLevelTestFactory.TunableFactory(), 'common_career': CareerCommonTestFactory.TunableFactory(), 'is_retired': IsRetiredTestFactory.TunableFactory(), 'has_career_outfit': HasCareerOutfit.TunableFactory(), 'time_until_work': CareerTimeUntilWorkTestFactory.TunableFactory(), 'pto_amount': CareerPTOAmountTestFactory.TunableFactory(), 'time_off': CareerTimeOffTest.TunableFactory(), 'highest_level_achieved': CareerHighestLevelAchievedTest.TunableFactory(), 'is_elective_course': CareerIsElectiveCourseTest.TunableFactory(), 'in_career_event': CareerEventTest.TunableFactory(), 'has_career_icon_override': CareerHasIconOverrideTest.TunableFactory(), 'default': 'career_reference'}
...
class _CareerEventOriginTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, career=None):
        ...
        if not self.event_origin_careers.test_item(type(career)):
...
class CareerPromotedTest(_CareerEventOriginTest):
    test_events = (TestEvent.CareerPromoted,)
...
class RewardPartTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'subject': TunableEnumEntry(description='\n            Who or what to apply this test to\n            ', tunable_type=ParticipantType, default=ParticipantType.Actor), 'reward_parts': TunableList(description='\n            All the reward parts that \n            ', tunable=TunableCasPart()), 'reward_parts_from_object_participant': TunableEnumEntry(description='\n            A participant whose saved cas parts will be appended to any tuned\n            in reward_parts.\n            ', tunable_type=ParticipantTypeCASPart, default=ParticipantTypeCASPart.StoredCASPartsOnObject), 'invert': Tunable(description='\n            If checked, test will pass if any subject does NOT have the unlock.\n            ', tunable_type=bool, default=False)}

    def get_expected_args(self):
        return {'test_targets': self.subject, 'additional_reward_parts': self.reward_parts_from_object_participant}
    ...
    def __call__(self, test_targets=(), additional_reward_parts=None):
        ...
        for target in test_targets:
            ...
            household = target.household
            sim_id = target.id
            if not all(household.part_in_reward_inventory(reward_part, sim_id) for reward_part in reward_parts):
                if self.invert:
                    return TestResult.TRUE
                return TestResult(False, "Sim {} hasn't unlock {}.", target, self.reward_parts, tooltip=self.tooltip)
        if self.invert:
            return TestResult(False, 'No subjects have {} locked', self.reward_parts, tooltip=self.tooltip)
        return TestResult.TRUE
...
class PhoneSilencedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self):
        ...
        if is_phone_silenced or self.is_silenced:
...
class DetectiveClueTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'participant': TunableEnumEntry(description='\n            The participant for which we are running the test.\n            ', tunable_type=ParticipantType, default=ParticipantType.Actor), 'career_reference': TunableReference(description='\n            The career for which we need to check clue information.\n            ', manager=services.get_instance_manager(sims4.resources.Types.CAREER), class_restrictions=('DetectiveCareer',)), 'threshold': TunableThreshold(description='\n            The number of clues required in order to pass this test.\n            ')}
...
class LockedPortalCountTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'number_to_test': TunableThreshold(description='\n            The number of doors that need to be locked to pass this test.\n            '), 'lock_priority': OptionalTunable(description='\n            The priority of the locks we want to test. None means all priorities.\n            ', tunable=TunableEnumEntry(tunable_type=LockPriority, default=LockPriority.PLAYER_LOCK), disabled_name='all_priorities', enabled_name='specify_priority'), 'lock_types': OptionalTunable(description='\n            The type of the locks we want to test. None means all types.\n            ', tunable=TunableEnumSet(enum_type=LockType, enum_default=LockType.LOCK_ALL_WITH_SIMID_EXCEPTION), disabled_name='all_lock_types', enabled_name='specify_lock_type')}
...
class PortalLockedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, target_list=None, sims_to_test=None):
        ...
        if lock_exist or not self.negate:
...
class HasPhotoFilterTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'participant': TunableEnumEntry(description='\n            The participant for which we are running the test.\n            ', tunable_type=ParticipantType, default=ParticipantType.Object), 'photo_filter': TunableEnumEntry(description='\n            The photo filter that you want to test that this photo is using.\n            ', tunable_type=PhotoStyleType, default=PhotoStyleType.NORMAL), 'negate': Tunable(description='\n            If checked the test will fail if the photo is using the tuned photo filter.\n            ', tunable_type=bool, default=False)}
...
class RegionTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def _check_region(self, region_to_test):
        if self.whitelist is not None and region_to_test not in self.whitelist:
            return TestResult(False, 'RegionTest: {} not in region whitelist.', region_to_test, tooltip=self.tooltip)
        if self.blacklist is not None and region_to_test in self.blacklist:
            return TestResult(False, 'RegionTest: {} in region blacklist', region_to_test, tooltip=self.tooltip)
        return TestResult.TRUE
    ...
    def __call__(self, sim_info_list=None):
        ...
        if not sim_info_list:
            return TestResult(False, 'RegionTest: sim_info_list is None when trying to check for regions.', tooltip=self.tooltip)
        import world
    ...
    def __call__(self, participants, clubs=None, targets=()):
        ...
        if self.bucks_perk is None:
            if self.test_if_unlocked:
                if not invert:
                    return TestResult(False, 'bucks_perk is None (likely pack not installed) and perk is not unlocked')
                return TestResult(True, 'bucks_perk is None (likely pack not installed) and perk is not unlocked, and invert is set to be True')
            if not invert:
                return TestResult(True, 'bucks_perk is None (likely pack not installed) and perk is locked')
            return TestResult(False, 'bucks_perk is None (likely pack not installed) and perk is locked, and invert is set to be True')
        if clubs is None and targets:
        ...
        for participant in participants:
            ...
            if clubs is not None:
                for club in clubs:
                    ...
                    if club.bucks_tracker in unlocked_trackers and not self.test_if_unlocked:
                        if not invert:
                ...
                if self.test_if_unlocked and not unlocked_trackers:
                    if not invert:
...
class EnsembleTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, test_targets=(), other_participants=None):
        ...
        for target in test_targets:
            ...
            if sim is None:
                return TestResult(False, 'EnsembleTest: Sim {} is not instanced.', target, tooltip=self.tooltip)
            if self.ensemble_option.check_type == ENSEMBLE_CHECK_ALL:
                ...
                logger.error('Trying to use EnsembleTest with an unhandled Ensemble Option {}', self.ensemble_option.check_type, owner='jjacobson')
            ...
            for ensemble in ensembles:
                ...
                if self.threshold.compare(target_number):
                    if self.invert:
                        return TestResult(False, 'EnsembleTest: Sim {} is in an ensemble {} that matches the tuned threshold.', sim, ensemble, tooltip=self.tooltip)
                    break
            if not self.invert:
                return TestResult(False, 'EnsembleTest: Sim {} is not in an ensemble that matches tuned threshold.', sim, tooltip=self.tooltip)
        return TestResult.TRUE

class PurchasePerkTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'bucks_type': TunableEnumEntry(description='\n            The participant of whos ensemble we want to look at.\n            ', tunable_type=BucksType, default=BucksType.INVALID, pack_safe=True), 'consider_existing_perks': Tunable(description='\n            If checked, this test will return true if the sim has already\n            purchased a perk with the tuned buck type.\n            ', tunable_type=bool, default=False)}
    test_events = (TestEvent.PerkPurchased,)

    def get_expected_args(self):
        return {'bucks_type': event_testing.test_constants.FROM_EVENT_DATA, 'perk': event_testing.test_constants.FROM_EVENT_DATA, 'participant': ParticipantType.Actor}
...
class TotalClubBucksEarnedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    test_events = (TestEvent.ClubBucksEarned,)
    USES_DATA_OBJECT = True
    FACTORY_TUNABLES = {'threshold': TunableThreshold(description='\n            Test for how may Club Bucks the Sim must earn to pass the test.\n            ', tunable_type=int, default=10)}

    def get_expected_args(self):
        return {'participant': ParticipantType.Actor, 'data_object': event_testing.test_constants.FROM_DATA_OBJECT}
...
class TimeInClubGatheringsTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    test_events = (TestEvent.TimeInClubGathering,)
    USES_DATA_OBJECT = True
...
class InteractionSourceTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, context=None):
        ...
        if context.source not in self.interaction_sources:
            result = TestResult(False, 'InteractionSourceTest failed. Interaction source: {}, is not in the tuned set: {}', context.source, self.interaction_sources, tooltip=self.tooltip)
        if self.invert:
            if result:
                return TestResult(False, 'InteractionSourceTest passed but invert is set to be True. Interaction source: {}, Tuned set: {}', context.source, self.interaction_sources, tooltip=self.tooltip)
            else:
                return TestResult.TRUE
        return result
...
class TunableTestBasedScoreTestVariant(TunableVariant):
    ...
    def __init__(self, **kwargs):
        tunables = {'relationship_test_based_score': RelationshipTestBasedScore.TunableFactory(), 'social_context': SocialContextTest.TunableFactory()}
        kwargs.update(tunables)
        super().__init__(**kwargs)

class TunableSituationMedalTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'who': TunableEnumEntry(description='\n            The Sim who is involved in the situation you want to test the score of.\n            ', tunable_type=ParticipantTypeSim, default=ParticipantTypeSim.Actor), 'situation': SituationSearchByDefinitionOrTagsVariant(description='\n            The situation you want to check the score for (by definition or by tag).\n            '), 'threshold': TunableThreshold(description='\n            The threshold to use when comparing the medal in order to determine whether or not the\n            test should pass.\n            ', value=TunableEnumEntry(description='\n                The value to test against.\n                ', tunable_type=SituationMedal, default=SituationMedal.TIN))}
    ...
    def __call__(self, *args, subjects=None, **kwargs):
        ...
        for subject in subjects:
            ...
            if sim is None:
                ...
                situations = self.situation.get_all_matching_situations()
                for situation in situations:
                    if situation.is_sim_in_situation(sim):
                        level = situation.get_level()
                        if self.threshold.compare(level):
                            return TestResult.TRUE
                        return TestResult(False, 'The Sim ({}) has a medal of {} instead of {} in the {} situation', subject, str(level), self.threshold, self.situation, tooltip=self.tooltip)
                return TestResult(False, 'The Sim ({}) was not in any situations of type {}', subject, self.situation, tooltip=self.tooltip)
        return TestResult(False, 'The subject Sims do not have a medal of {} in the {} situation.', self.threshold, self.situation, tooltip=self.tooltip)

class MatchTuningReferenceTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):

    class _MatchRule(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'rule_to_match': TunablePackSafeReference(manager=services.get_instance_manager(sims4.resources.Types.BUSINESS_RULE))}

        def test_match(self, subject:'Any') -> 'bool':
            if subject is None or self.rule_to_match is None:
                return False
            return self.rule_to_match.guid64 == subject

    class _MatchDramaNode(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'drama_node_to_match': TunablePackSafeReference(manager=services.get_instance_manager(sims4.resources.Types.DRAMA_NODE))}

        def test_match(self, subject:'Any') -> 'bool':
            if subject is None or self.drama_node_to_match is None:
                return False
            return self.drama_node_to_match.guid64 == subject

    FACTORY_TUNABLES = {'subject': TunableEnumEntry(description='\n            The participant to be compared to tuning reference.\n            ', tunable_type=ParticipantType, default=ParticipantType.PickedItemId), 'test_type': TunableVariant(description='\n            The test to perform. Check if subject is matching tuning reference.\n            ', check_match_rule=_MatchRule.TunableFactory(description='\n                Return True if subject is equal to rule reference.\n                '), check_match_drama_node=_MatchDramaNode.TunableFactory(description='\n                Return True if subject is equals to drama node reference.\n                '), default='check_match_rule')}

    def get_expected_args(self):
        return {'subjects': self.subject}

    def __call__(self, subjects:'Set[Any]') -> 'TestResult':
        test_subject = next(iter(subjects), None)
        if test_subject is None:
            return TestResult(False, "There isn't a valid subject to compare with", tooltip=self.tooltip)
        result = self.test_type.test_match(test_subject)
        if result:
            return TestResult.TRUE
        return TestResult(False, "Subject {} doesn't match reference", test_subject, tooltip=self.tooltip)

class BusinessTestEvents(enum.Int):
    Register = event_testing.test_events.TestEvent.BusinessRegistered
    FirstSimoleonEarned = event_testing.test_events.TestEvent.BusinessFirstSimoleonEarned
    Sold = event_testing.test_events.TestEvent.BusinessSold

class BusinessTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    USES_DATA_OBJECT = True
    USES_EVENT_DATA = True
    FACTORY_TUNABLES = {'description': 'Check to see if the business triggered its milestone events', 'business_type': TunableEnumEntry(description='\n            The business type to which this reward should be given.\n            ', tunable_type=BusinessType, default=BusinessType.INVALID, invalid_enums=(BusinessType.INVALID,)), 'test_event': TunableEnumEntry(description='\n            The event that we want to trigger this instance of the tuned\n            test on.\n            Register: Triggers when business is registered\n            FirstSimoleonEarned = Triggers when a business earned first simoleon\n            Sold = Triggers when a business is sold\n            ', tunable_type=BusinessTestEvents, default=None)}

    def get_test_events_to_register(self):
        return (self.test_event,)

    def get_expected_args(self):
        return {'event_business_type': event_testing.test_constants.FROM_EVENT_DATA}

    @cached_test
    def __call__(self, event_business_type=None):
        if event_business_type:
            if self.business_type != event_business_type:
                return TestResult(False, 'BusinessTest: Business event {} for business type {} did not match the event business type {}', self.test_event, self.business_type, event_business_type)
            return TestResult.TRUE
        else:
            return TestResult(False, 'BusinessTest: Business event {} for business type {} did not have the event data', self.test_event, self.business_type)

class LotOwnershipByVenueTypeTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'participant': TunableEnumEntry(description='\n            The participant that we want to check owns a lot of the given Venue type.\n            ', tunable_type=ParticipantTypeSingleSim, default=ParticipantTypeSingleSim.Actor), 'venue_type': TunablePackSafeReference(description='\n            The venue lot type the sim should own at least 1 of.\n            ', manager=services.get_instance_manager(sims4.resources.Types.VENUE)), 'negate': Tunable(description='\n            If checked, test will pass if the Sim does NOT own a Venue of the given type.\n            ', tunable_type=bool, default=False)}

    def get_expected_args(self):
        return {'owner_sim': self.participant}

    @cached_test
    def __call__(self, owner_sim=None):
        if not owner_sim:
            return TestResult(False, 'LotOwnershipByVenueTypeTest: Unable to fetch sim info', tooltip=self.tooltip)
        sim_info = owner_sim[0]
        venue_managers = services.venue_service()
        zone_ids = venue_managers.get_zones_for_venue_type_gen(self.venue_type)
        if zone_ids:
            for zone_id in zone_ids:
                ...
                if zone_data is not None:
                    lot_data = persistence_service.get_lot_data_from_zone_data(zone_data)
                    for lot_owner in lot_data.lot_owner:
                        if lot_owner.household_id == sim_info.household_id:
                            if self.negate:
                                return TestResult(False, 'LotOwnershipByVenueTypeTest: Test is negated. {} OWNS a {}', sim_info, self.venue_type, tooltip=self.tooltip)
                            return TestResult.TRUE
        if self.negate:
            return TestResult.TRUE
        return TestResult(False, "LotOwnershipByVenueTypeTest: {} doesn't own a {}", sim_info, self.venue_type, tooltip=self.tooltip)

class CanStartPivotalMomentTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'negate': Tunable(description='\n            If checked, test will return true when the maximum number of pivotal moments has been reached.\n            ', tunable_type=bool, default=False)}
    ...
    def __call__(self):
        ...
        if tutorial_service.can_new_pivotal_moment_start(None):
            if self.negate:
                return TestResult(False, 'CanStartPivotalMomentsTest: Test Negated, pivotal moment CAN be started', tooltip=self.tooltip)
            return TestResult.TRUE
        if self.negate:
            return TestResult.TRUE
        return TestResult(False, 'CanStartPivotalMomentsTest: Max number of active Pivotal Moments has been reached', tooltip=self.tooltip)

class DiscoveryQuestsEnabledTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'negate': Tunable(description='\n            If checked, test will return true if the player has pivotal moments disabled.\n            ', tunable_type=bool, default=False)}
    ...
    def __call__(self):
        tutorial_service = services.get_tutorial_service()
        if tutorial_service.pivotal_moments_enabled:
            if self.negate:
                return TestResult(False, 'DiscoveryQuestsEnabledTest: Test Negated, discovery quests are enabled.', tooltip=self.tooltip)
            return TestResult.TRUE
        if self.negate:
            return TestResult.TRUE
        return TestResult(False, 'DiscoveryQuestsEnabledTest: Discovery Quests are NOT enabled.', tooltip=self.tooltip)

class DiscoveryQuestCompletedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'pivotal_moment': TunablePackSafeReference(description='\n            The pivotal moment/discovery quest to test.\n            ', manager=services.get_instance_manager(sims4.resources.Types.SNIPPET), class_restrictions='PivotalMoment')}
    ...
    def __call__(self):
        ...
        if tutorial_service is None:
            return TestResult(False, 'Tutorial service not loaded yet.', tooltip=self.tooltip)
        if tutorial_service.pivotal_moments_enabled:
            if tutorial_service.is_pivotal_moment_completed(self.pivotal_moment.guid64):
                return TestResult.TRUE
            return TestResult(False, 'DiscoveryQuestCompletedTest: Pivotal Moment not yet completed.', tooltip=self.tooltip)
        else:
            return TestResult(False, 'DiscoveryQuestCompletedTest: Pivotal Moments not enabled.', tooltip=self.tooltip)
