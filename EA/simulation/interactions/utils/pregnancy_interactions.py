...
class DeliverBabySuperInteraction(SuperInteraction, NameOffspringSuperInteractionMixin):
    ...
    def _create_new_bassinet_with_baby(self, pregnancy_tracker:'PregnancyTracker', offspring_data:'PregnancyOffspringData', empty_bassinet:'GameObject', allow_override:'bool'=False) -> 'Optional[GameObject]':
        ...
        if allow_override:
            for entry in BabyTuning.BABY_DEFAULT_BASSINETS:
                if not entry.bassinets:
                    pass
                elif any(sim_info.has_trait(trait) for trait in entry.traits):
                    return set_baby_sim_info_with_switch_id(empty_bassinet, sim_info, True)
        return set_baby_sim_info_with_switch_id(empty_bassinet, sim_info)
    ...
    def _pregnancy_completion(self, pregnancy_tracker:'PregnancyTracker', allow_override:'bool'=False) -> 'bool':
        ...
        if is_bassinet_birth:
            new_target = self._create_new_bassinet_with_baby(pregnancy_tracker, offspring_data_list[0], self.target, allow_override)
    ...
    def _complete_pregnancy_gen(self, timeline:'Timeline', pregnancy_tracker:'PregnancyTracker') -> 'Iterator[bool]':
        return self._pregnancy_completion(pregnancy_tracker)
...
class DeliverBabyOverridableBassinetInteraction(DeliverBabySuperInteraction):

    def _complete_pregnancy_gen(self, timeline:'Timeline', pregnancy_tracker:'PregnancyTracker') -> 'Iterator[bool]':
        return self._pregnancy_completion(pregnancy_tracker, True)
