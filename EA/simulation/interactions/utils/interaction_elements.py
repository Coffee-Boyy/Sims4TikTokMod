...
class XevtTriggeredElement(elements.ParentElement, HasTunableFactory, AutoFactoryInit):
    ...
    AT_END = 'at_end'
    ON_XEVT = 'on_xevt'
    CRITICALITY_DESC = '\n        Determines the condition in which it will run.\n        \n        NotCritical - If the interaction is cancelled or reset out of, it will not run.\n        OnCancel - If the interaction is cancelled, it will run, but not if it is reset out of.\n        OnCancelOrException - If the interaction is cancelled or reset out of, it will run.\n        RunAll - It will always run.\n        '
...
class PutNearElement(XevtTriggeredElement):
    ...
    def _do_behavior(self, *args, **kwargs) -> 'None':
        ...
        if self.use_world_routing and self.offset:
            offset_transform = sims4.math.Transform(self.offset.offset_translation, sims4.math.angle_to_yaw_quaternion(self.offset.offset_orientation))
        InteractionUtils.do_put_near(subject, target, self.fallback_to_spawn_point, self.use_fgl, offset=offset_transform)
