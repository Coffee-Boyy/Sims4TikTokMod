...
class WaypointInteraction(SuperInteraction):
    ...
    def should_cancel_on_si_cancel(self, interaction):
        left_carry = self.sim.posture_state.left
        if self.sim.species == Species.HORSE and (interaction.user_canceled and (interaction is left_carry.source_interaction and left_carry.target is not None)) and left_carry.target.is_sim:
            return True
        return False
    ...
    def _run_interaction_gen(self, timeline):
        ...
        default_max_goal_clipping_score = None
        path_plan_context = routing_agent.routing_component._pathplan_context
        if routing_agent.routing_component is not None:
            if self.waypoint_override_agent_radius is not None:
                default_agent_radius = path_plan_context.agent_radius
                path_plan_context.agent_radius = self.waypoint_override_agent_radius
            if self.max_goal_clipping_score_override is not None:
                default_max_goal_clipping_score = path_plan_context.max_goal_clipping_score
                path_plan_context.max_goal_clipping_score = self.max_goal_clipping_score_override
        try:
            ...
            if default_agent_radius is not None:
                path_plan_context.agent_radius = default_agent_radius
            if default_max_goal_clipping_score is not None:
                path_plan_context.max_goal_clipping_score = default_max_goal_clipping_score
        if not waypoints:
            return False
