        ...
        if self.is_complete:
            ...
            if carry is sim:
                ...
                if carry is not None and path_left[-1].carry_target is None and carry is None or isinstance(carry, Definition):
                    ...
                    spec = sim.posture_state.spec
                    left_destinations.add(spec)
...
class TransitionSpecCython:
    ...
    __annotations__['_additional_reservation_handlers'] = list
    handle_slot_reservations = cython.declare(cython.bint, visibility='public')
    __annotations__['_portal_ref'] = object
    ...
    def targets_empty_slot(self):
        ...
        if surface_spec.slot_type is not None and surface_spec.slot_target is None:
            return True
        return False
        ...
        try:
            ...
            if retrieve_posture_objects.transition_retrieval_affordance is not None:
                ...
                if objects_to_retrieve:
                    self.handle_slot_reservations = False
            if retrieve_posture_objects is not None and self.handle_slot_reservations:
                object_to_ignore = []
    ...
    def get_transition_route(self, sim, fade_out, lock_out_socials, dest_posture, interaction):
        ...
        def route_callback(distance_left):
            ...
            if self.is_failure_path or distance_left < FollowPath.DISTANCE_TO_RECHECK_STAND_RESERVATION and (sim.species == Species.HORSE or sim.routing_component.on_slot is not None):
            ...
            if distance_left < TransitionSpec.DISTANCE_TO_FADE_SIM_OUT:
                ...
                if lock_out_socials:
                    ...
                    def _should_cancel_interaction(inter):
                        return not inter.involves_carry and (not inter.is_carry_cancel_interaction and (not inter.is_pickup_requester and (not inter.is_putdown and (not inter.paired_horse_transition_state and (not inter.is_waiting_pickup_putdown and (not inter.is_super or not inter.is_posture_provider))))))

                    def _parent_is_running_interaction(inter, child_sim, parent_sim):
                        return child_sim.parent is parent_sim and inter is parent_sim.queue.running

                    def _sim_involved(inter):
                        target = inter.target
                        return inter.has_sim_in_required_sim_cache(sim) or target is not None and (target.is_sim and target is sim)
                    ...
                    for other_sim in master_controller.added_sims():
                        if other_sim.is_sim:
                            if other_sim is sim:
                                ...
                                queued_interactions = other_sim.queue
                                for queued_interaction in queued_interactions:
                                    if _sim_involved(queued_interaction) and _should_cancel_interaction(queued_interaction) and not _parent_is_running_interaction(queued_interaction, other_sim, sim):
                                        other_sim.remove_interaction_reference(queued_interaction)
                                        sim.remove_interaction_reference(queued_interaction)
                                        queued_interaction.cancel(finishing_type=FinishingType.OBJECT_CHANGED, cancel_reason_msg='Queued interaction cancelled from an involved Sim preparing to be destroyed.', ignore_must_run=True, immediate=True)
        ...
        def should_fade_in():
            if interaction is not None and interaction.get_liability(SpawnActionLiability.LIABILITY_TOKEN):
                return False
            elif not fade_sim_out:
...
class PathSpec:
    ...
    def attach_route_and_params(self, path, locked_params, final_constraint, reverse=False):
        ...
        for transition_spec in sequence:
            ...
            if carry_spec is None and route_spec is None:
                if reverse or transition_spec.posture_spec.body.target is not None:
                    route_spec = previous_spec if previous_spec is not None else transition_spec
                elif previous_spec.posture_spec.carry.target is not None != transition_spec.posture_spec.carry.target is not None:
    ...
    def finalize(self, sim):
        if self.path is not None and len(self.path) > 0:
            ...
            if body_posture_target is not None and sim.posture.mobile and self.path[0].body.target != body_posture_target:
    ...
    def remove_non_surface_to_surface_transitions(i, prev_transition_spec, transition_spec, next_transition_spec):
        ...
        if next_transition is not None and ((prev_transition is None or prev_transition.body.posture_type.mobile) and (transition.surface.target is None and (next_transition.body.posture_type.mobile and next_transition.surface.target is not None))) and (prev_transition is None or prev_transition.carry == next_transition.carry or next_transition is None):
            return ()
    ...
    def remove_extra_mobile_transitions(i, prev_transition_spec, transition_spec, next_transition_spec):
        ...
        if prev_transition is None or next_transition is None:
            return (transition_spec,)
        if prev_transition.carry == next_transition.carry and (prev_transition.surface == next_transition.surface and prev_transition.body.target == next_transition.body.target) and transition.body.posture_type == next_transition.body.posture_type:
            return ()
        ...
        if prev_transition.carry.target != transition.carry.target or transition.carry.target != next_transition.carry.target:
            return (transition_spec,)
        if prev_transition.body.posture_type.mobile or next_transition_spec.path is not None:
            return (transition_spec,)
        if prev_transition.body.posture_type.mobile and (transition.body.posture_type.mobile and next_transition.body.posture_type.mobile) and services.current_zone().posture_graph_service._can_transition_between_nodes(prev_transition, next_transition):
            return ()
        return (transition_spec,)
    ...
    def flag_slot_reservations(self):
        for (prev_transition_spec, cur_transition_spec) in zip(self._path, self._path[1:]):
            if prev_transition_spec.sequence_id != cur_transition_spec.sequence_id:
                pass
            elif not cur_transition_spec.posture_spec.surface.target:
                pass
...
class PostureGraph:
    ...
    cached_vehicle_nodes = cython.declare(dict, visibility='readonly')
    ...
    def remove_node(self, node:PostureSpec):
        ...
        if target != PostureSpecVariable.ANYTHING:
            ...
            if body_type.is_vehicle:
                self.cached_vehicle_nodes[target.guid64].remove(node)
                if len(self.cached_vehicle_nodes[target.guid64]) == 0:
                    del self.cached_vehicle_nodes[target.guid64]
    ...
    def _add_node(self, node:PostureSpec):
        ...
        if not target.is_sim:
            ...
            body_type = node.body.posture_type
            if body_type.is_vehicle:
                if target.guid64 in self.cached_vehicle_nodes:
                    self.cached_vehicle_nodes[target.guid64].add(node)
                else:
                    self.cached_vehicle_nodes[target.guid64] = {node}
        for key in get_subset_keys(node):
    ...
    def nodes_matching_constraint_geometry(self, constraint):
        ...
        nodes |= self._subsets[('body_target', self.proxy_sim)]
        return nodes
    ...
    def vehicle_nodes_gen(self):
        for nodes in self.cached_vehicle_nodes.values():
            for node in nodes:
                yield node
...
class EdgeInfo(namedtuple('_EdgeInfo', ['operations', 'validate', 'species_to_cost_dict', 'cost_calculation_history'])):
    __slots__ = ()
    ...
    @property
    def cost_reason(self) -> str:
        if self.cost_calculation_history is None:
            return 'Cost calculation history unavailable, please rebuild posture graph (|posture_graph.build)'
        return ', '.join(self.cost_calculation_history)
...
class PostureGraphService(Service):
    ...
    def get_default_affordance(species=Species.HUMAN, sim_info=None):
        if sim_info is not None:
            for data in PostureGraphService.SIM_DEFAULT_AFFORDANCES_OVERRIDES:
                if data.age:
                    if data.age == sim_info.age:
                        resolver = SingleSimResolver(sim_info)
                        if data.tests.run_tests(resolver):
                            return data.affordance
                resolver = SingleSimResolver(sim_info)
                if data.species == species and data.tests.run_tests(resolver):
                    return data.affordance
    ...
    def add_node(self, node:PostureSpec, operations):
        ...
        cost_calculation_history = None
        validate = CallableTestList()
        save_edge_calculation = gsi_handlers.posture_graph_handlers.archiver.enabled
        if save_edge_calculation:
        ...
        for (source_node, operation) in zip(nodes, operations):
            if species_to_cost_dict is None:
                ...
                if save_edge_calculation:
                    edge_calc_reason = 'Sourcing cost info from operation {}'.format(type(operation).__name__)
                    operation_cost_str_list = operation.debug_cost_str_list
                    if operation_cost_str_list is not None:
                        edge_calc_reason += ': ' + ', '.join(operation_cost_str_list)
                    cost_calculation_history.append(edge_calc_reason)
            else:
                ...
                for species in species_to_cost_dict:
                    if species in temp_cost_dict:
                        temp_species_cost = temp_cost_dict.pop(species)
                        if save_edge_calculation:
                            cost_calculation_history.append('Adding existing cost {} to {} for {} from {}'.format(temp_species_cost, species_to_cost_dict[species], species, type(operation).__name__))
                        species_to_cost_dict[species] += temp_species_cost
                    else:
                        if save_edge_calculation:
                            cost_calculation_history.append('Increment {} by default opcost {} for {} from {}'.format(species_to_cost_dict[species], new_default_cost, species, type(operation).__name__))
                        species_to_cost_dict[species] += new_default_cost
                for species in temp_cost_dict:
                    new_species_cost = temp_cost_dict.pop(species)
                    species_to_cost_dict[species] = current_default_cost + new_species_cost
                    if save_edge_calculation:
                        cost_calculation_history.append('Add edge default cost {} to opcost {} = {} for {} from {}'.format(current_default_cost, new_species_cost, species_to_cost_dict[species], species, type(operation).__name__))
            validate.append(operation.get_validator(source_node))
        edge_info = EdgeInfo(operations, validate, species_to_cost_dict, cost_calculation_history)
    ...
    def object_moving(self, obj):
        ...
        if obj.routing_component is not None and obj.routing_component.is_moving:
            ...
            yield None
            return
        if not obj.should_rebuild_posture_graph_for_snap_event:
            yield None
            return
    ...
    def _expand_node(self, node:PostureSpec):
        ...
        if node.body.posture_type._supports_infant and SIM_INFANT_OVERRIDE_DEFAULT_OPERATION is not None:
    ...
    def _is_eligible_generic_carry_sim(self, sim):
        if sim.sim_info.is_human:
            pass
        return sim.sim_info.species == Species.HORSE
    ...
    def _expand_and_append_node_sim(self, node, obj, out_ops):
        ...
        if not species_to_affordances:
            return
        if not self._is_eligible_generic_carry_sim(obj):
            return
    ...
    def _validate_node_for_graph(node):
        ...
        if target is not None and not node.body.posture_type.mobile:
            surface = node.surface
    ...
    def build(self):
        ...
        self._edge_info[(STAND_AT_NONE, STAND_AT_NONE)] = EdgeInfo((SIM_DEFAULT_OPERATION,), lambda *_, **__: True, 0, 'Stand@None -> Stand@None')
    ...
    def distance_fn(self, sim, var_map, interaction, ensemble, curr_node, next_node):
        ...
        if next_body_target is not None:
            if next_body_target.is_sim:
                ...
                if ensemble is not None and not ensemble.is_sim_in_ensemble(next_body_target):
                    cost += PostureScoring.CARRYING_SIM_NOT_IN_ENSEMBLE_PENALTY
                    if gsi_handlers.posture_graph_handlers.archiver.enabled:
                        cost_str_list.append('CARRYING_SIM_NOT_IN_ENSEMBLE_PENALTY: {}'.format(PostureScoring.CARRYING_SIM_NOT_IN_ENSEMBLE_PENALTY))
                edge_cost *= curr_node.body_posture.get_target_species_cost_modifier(next_body_posture, next_body_target.species)
            elif not may_reserve_posture_target(sim, next_body_posture, next_body_target):
        ...
        if gsi_handlers.posture_graph_handlers.archiver.enabled:
            ...
            cost_str_list.append('OpCost: {}'.format(edge_cost))
            cost_str_list.append('Notes: {}'.format(edge_info.cost_reason))
            for operation in edge_info.operations:
                cost_str_list.append('\nOperation: {}'.format(type(operation).__name__))
                operation_cost_str_list = operation.debug_cost_str_list
                if operation_cost_str_list is not None:
                    cost_str_list.append(', '.join(operation_cost_str_list))
    ...
    def _adjacent_nodes_gen(self, sim, get_successors_fn, valid_edge_test, constraint, var_map, node, *, allow_pickups, allow_putdowns, allow_carried, reverse_path, force_carry_path):
        node = node.graph_node
        ...
        for sub_constraint in constraint:
            if sub_constraint.posture_state_spec is None:
                ...
                posture_specs = [posture_spec for (posture_spec, var_map) in sub_constraint.posture_state_spec.get_posture_specs_gen() if posture_spec.body_posture == node.body.posture_type]
                for spec in posture_specs:
                    if spec.body.target in PostureSpecVariable or node.body.target is spec.body.target:
                        ...
                        break
        skip_adjacent = (reverse_path or node.body_posture.is_vehicle and is_mobile_at_none or is_routing_vehicle) and not force_carry_path
        skip_adjacent = False
        ...
        def _edge_validation(forward_nodes, successor_node):
            ...
            if successor_node.body_target is not None and successor_node.body_target.is_disabled():
                return
            if not (force_carry_path and (successor_node.body_target is None or successor_node.body_target.is_sim or node_body_target is None or node_body_target.is_sim)):
                return
            ...
            if not skip_adjacent or not body_target is None:
                if not body_target.is_sim:
                    ...
                    if allow_pickups or not first.carry_target is None or second.carry_target is not None:
                        ...
                        for carry_successor in self._graph.sim_carry_nodes_gen(successor):
                            if node.body_target is carry_successor.body_target:
                                pass
                            else:
                                carry_forward_nodes = (carry_successor, node) if reverse_path else (node, carry_successor)
                                ...
                                if allow_pickups or not first_carry.carry_target is None or second_carry.carry_target is not None:
                                    ...
                                    return_node = _edge_validation(carry_forward_nodes, carry_successor)
    ...
    def _left_path_gen(self, sim, source, destinations, interaction, constraint, var_map, valid_edge_test, is_complete, force_carry_path):
        ...
        if is_complete:
            if source.body.posture_type.mobile and source.surface.target is None and (source.body.posture_type.is_vehicle or all(dest.body.target != source.body.target for dest in destinations)):
                return
            ...
            if sim.is_human:
                ...
                if source.body_posture.is_vehicle and source in destinations:
                    if source.body_target.guid64 in self._graph.vehicle_nodes:
                        search_destinations.update(self._graph.vehicle_nodes[source.body_target.guid64])
                else:
        ...
        adjacent_nodes_gen = functools.partial(self._adjacent_nodes_gen, sim, self._graph.get_successors, valid_edge_test, constraint, var_map, reverse_path=False, allow_pickups=allow_pickups, allow_putdowns=allow_putdowns, allow_carried=allow_carried, force_carry_path=force_carry_path)
    ...
    def _get_goal_cost(self, sim, interaction, constraint, var_map, dest):
        ...
        if not (node_target.is_sim and node_target.species == Species.HORSE):
            cost += constraint.constraint_cost(node_target.position, node_target.orientation)
        if not (node_target is not None and (dest.body.posture_type.is_vehicle or any(c.cost for c in constraint))):
            return cost
    ...
    def _right_path_gen(self, sim, interaction, distance_estimator, left_destinations, destinations, var_map, constraint, valid_edge_test, path_left, *, allow_carried, ensemble):
        adjacent_nodes_gen = functools.partial(self._adjacent_nodes_gen, sim, self._graph.get_predecessors, valid_edge_test, constraint, var_map, reverse_path=True, allow_pickups=False, allow_putdowns=True, allow_carried=allow_carried, force_carry_path=False)

        def reversed_distance_fn(curr_node, next_node):
            if next_node is None:
                return 0.0
            return self.distance_fn(sim, var_map, interaction, ensemble, next_node, curr_node)
        ...
        paths_reversed = _shortest_path_gen(sim, weighted_sources, set(left_destinations), False, adjacent_nodes_gen, reversed_distance_fn, heuristic_fn_right)
    ...
    def _middle_path_gen(self, path_left, path_right, sim, interaction, distance_estimator, var_map):
        ...
        if carry_target.is_sim:
            ...
            if parent_slot.owner.is_sim:
                return []
            if not (parent_slot is not None and parent_slot.owner is not sim and carry_target.parented_to_routable_object):
                surface_target = parent_slot.owner
                if not surface_target.is_surface(ignore_deco_slots=False):
    ...
    def get_segmented_paths(self, sim, posture_dest_list, additional_template_list, interaction, participant_type, valid_destination_test, valid_edge_test, preferences, final_constraint, included_sis):
        ...
        for (constraint, templates) in posture_dest_list.items():
            ...
            if source_spec is None:
                ...
                if not (source_spec.body_posture.mobile and (source_spec.body_posture.unconstrained and source_spec.body_target is not None) and (source_spec.body_target.is_part or graph.contains(source_spec))):
                ...
                for node in graph.get_matching_nodes_gen(destination_specs, slot_types, constraint=matching_node_constraint):
                            ...
                            if same_mobile_body_target and (node.body.target is not None and node.body.target is not source_node.body_target) and interaction.is_putdown:
                                ...
                                if obj is not None:
                                    ...
                                    if not valid:
                                        ...
                                        if additional_template_list and (interaction.is_putdown or interaction.needs_compatible_destination_template):
                                            compatible = True
        ...
        for (carry_si, additional_templates) in additional_template_list.items():
            ...
            if carry_si_carryable is None and carry_si.target is not None and carry_si_carryable is final_carry_target:
                                        ...
                                        if container is not sim:
                                            ...
                                            for (prev_transition_spec, transition_spec) in zip(best_transition_specs, best_transition_specs[1:]):
                                                ...
                                                if transition_spec.sequence_id != prev_transition_spec.sequence_id:
                                                    ...
                                                    if fallback_insertion_index_and_spec is None and prev_transition_spec.posture_spec.carry.target is None and spec.carry.target is not None:
                                            ...
                                            if fallback_insertion_index_and_spec is not None:
                                                ...
                                                pick_up_sequence = get_pick_up_spec_sequence(spec, container, body_target=spec.body.target)
                                                ...
                                                for pick_up_spec in pick_up_sequence:
                                                    new_specs.append(TransitionSpecCython_create(best_path_spec, pick_up_spec, var_map_additional_updated, sequence_id=SequenceId.PICKUP))
                                                            ...
                                                            for (prev_transition_spec, transition_spec) in zip(best_transition_specs, best_transition_specs[1:]):
                                                                ...
                                                                if transition_spec.sequence_id != prev_transition_spec.sequence_id:
                                                                    ...
                                                                    if spec.surface.target is slot_manifest_entry.actor.parent:
                                                                        fallback_insertion_index_and_spec = InsertionIndexAndSpec(insertion_index, spec)
                                                                    if fallback_insertion_index_and_spec is None and prev_transition_spec.posture_spec.carry.target is not None and spec.carry.target is None:
                                                                        break
                                                            if fallback_insertion_index_and_spec is not None:
                                                                insertion_index = fallback_insertion_index_and_spec.index
                                                                spec = fallback_insertion_index_and_spec.spec
                                                                put_down_sequence = get_put_down_spec_sequence(spec.body.posture_type, spec.surface.target, body_target=spec.body.target)
                                                                new_specs = []
                                                                for put_down_spec in put_down_sequence:
                                                                    new_specs.append(TransitionSpecCython_create(best_path_spec, put_down_spec, var_map_additional, sequence_id=SequenceId.PUTDOWN))
                ...
                if not valid_additional_intersection:
                    if can_defer_putdown and carry_si_carryable.carryable_component.defer_putdown or is_wing_proxy_object(carry_si_carryable):
    ...
    def get_sim_position_routing_data(self, sim, interaction):
        if sim.parent is not None and (sim.parent.is_sim or sim.posture_state.body.is_vehicle):
            return self.get_sim_position_routing_data(sim.parent, interaction)
        objects_to_ignore = set()
        jig_liability = interaction.get_liability(ObjectJigConstraint.JIG_CONSTRAINT_LIABILITY)
        if sim in jig_liability.ignore_sims_ref:
            objects_to_ignore = {jig_liability.jig.id}
        sim_position_constraint = interactions.constraints.Transform(sim.intended_transform, routing_surface=sim.intended_routing_surface, objects_to_ignore=objects_to_ignore, debug_name='SimCurrentPosition')
    ...
    def append_handles(sim, handle_dict, invalid_handle_dict, invalid_los_dict, routing_data, target_path, var_map, dest_spec, cur_path_id, final_constraint, entry=True, path_type=PathType.LEFT, goal_height_limit=None, perform_los_check=True):
        ...
        is_carry_target = target is carry_target or target is not None and (target.is_sim and target.is_riding_horse)
        (reference_pt, top_level_parent) = Constraint.get_los_reference_point(target, is_carry_target=is_carry_target)
        if not (target is None or target.is_sim and target.is_riding_horse):
            goal_height_limit = None
            if top_level_parent is not sim:
                (reference_pt, top_level_parent) = (None, None)
        if not (path_type == PathType.RIGHT and target is carry_target and perform_los_check):
            reference_pt = None
    ...
    def _generate_left_handles(self, sim, interaction, participant_type, left_path, var_map, destination_spec, final_constraint, unique_id, sim_position_routing_data):
        ...
        invalid_los = {}
        blocking_obj_ids = []
        if left_path[0].body.posture_type.use_containment_slot_for_exit or not left_path[0].body.posture_type.multi_sim_generic_object:
            ...
            if left_path[0].body.posture_type.multi_sim_generic_object:
                sim.posture.register_interaction_for_derailment(interaction)
    ...
    def _generate_right_handles(self, sim, interaction, participant_type, right_path, var_map, destination_spec, final_constraint, unique_id, animation_resolver_fn):
        ...
        if not (first_spec.body.posture_type.mobile and (first_spec.body.target is None or first_spec.body.posture_type.unconstrained)):
            ...
            if final_spec.body_posture in PostureTuning.MULTI_SIM_ENTRY_AND_EXIT_POSTURES:
                relevant_interaction = interaction if entry_spec.body_target is final_spec.body_target else None
            ...
            if relevant_interaction is None and use_previous_pos:
        ...
        if constrained_edge is None:
            ...
            if carry_transition_constraint is not None:
                ...
                if constraint_has_geometry:
                    overrides_multi_surface = relative_object.override_multi_surface_constraints
                    if relative_object.parent is not None:
                        overrides_multi_surface = relative_object.parent.override_multi_surface_constraints_of_children
                    carry_transition_constraint = carry_transition_constraint.generate_single_surface_constraints(override_multi_surface=overrides_multi_surface)
                    ...
                    if overrides_multi_surface is None and blocking_obj_id is not None:
                        blocking_obj_ids.append(blocking_obj_id)
                else:
                    self.copy_handles(sim, middle_handles, middle_path, var_map)
                        ...
                        if entry_node is not None and entry_node.body_target in searched[PathType.RIGHT]:
                            ...
                            if not destination_handles:
                                ...
                                if final_body_target is not None and final_body_posture.is_vehicle and final_body_posture is not final_left_node.body_posture:
                                    ...
                                    slot_constraint = sim_position_routing_data[0] if final_body_target.is_sim else posture.slot_constraint_simple
    ...
    def generate_connectivity_handles(self, sim, segmented_paths, interaction, participant_type, animation_resolver_fn, force_carry_path):
        ...
        sim_position_routing_data = self.get_sim_position_routing_data(sim, interaction)
        best_complete_path = EMPTY_PATH_SPEC
        for segmented_path in segmented_paths:
            if not segmented_path.is_complete:
                pass
            else:
                for left_path in segmented_path.generate_left_paths(force_carry_path):
                        ...
                        if not self.is_valid_complete_path(sim, complete_path, interaction):
                            ...
                            if len(complete_path) == 1:
                                ...
                                if sim.parent is not None and sim.parent.is_sim:
                                    transform_constraint = interactions.constraints.Transform(sim.parent.transform, routing_surface=sim.parent.routing_surface)
                                    geometry_constraint = segmented_path.constraint.generate_geometry_only_constraint()
                                    if not transform_constraint.intersect(geometry_constraint).valid:
                                        pass
                                    else:
        ...
        for segmented_path in segmented_paths:
            if segmented_path.is_complete:
                ...
                try:
                    handles = self._get_segmented_path_connectivity_handles(sim, segmented_path, interaction, participant_type, animation_resolver_fn, sim_position_routing_data, force_carry_path)
    ...
    def get_best_path_between_handles(self, interaction, sim, source_destination_sets, timeline, path_type=None):
                    ...
                    if path_type != PathType.MIDDLE_RIGHT and (same_interaction_source_target and (sim_surface_type is not None and (sim_surface_type != routing.SurfaceType.SURFACETYPE_POOL and source_goal.routing_surface_id.type != routing.SurfaceType.SURFACETYPE_POOL))) and sim_surface_type != source_goal.routing_surface_id.type:
                        ...
                        if not source_handle.target.is_sim:
                            ...
                            if routing.test_point_placement_in_navmesh(goal_routing_surface, goal_transform.translation) and sim.validate_location(sims4.math.Location(goal_transform, goal_routing_surface)):
                                (result, _) = source_handle.target.check_line_of_sight(goal_transform, verbose=True, for_carryable=for_carryable)
        ...
        for (_, destination_handles, _, _, _, _) in source_destination_sets.values():
            for (destination_handle, (right_path, path_cost, var_map, dest_spec, _, _, _)) in destination_handles.items():
                ...
                for dest_handle in additional_dest_handles:
                    ...
                    slope_restricted = target_reference_override is None and interaction.carry_target is dest_handle.target and dest_handle.target is not None and (dest_handle.constraint.restricted_on_slope and not interaction.ignore_slope_restrictions)
        ...
        original_agent_radius = None
        if resolver is not None:
            for agent_participant_type in interaction.agent_radius_overrides:
                agent = resolver.get_participant(agent_participant_type)
                if not agent is routing_agent:
                    if agent.is_sim and agent is routing_agent.sim_info:
                        original_agent_radius = routing_agent.routing_context.agent_radius
                        routing_agent.routing_context.agent_radius = interaction.agent_radius_overrides[agent_participant_type]
                        break
                original_agent_radius = routing_agent.routing_context.agent_radius
                routing_agent.routing_context.agent_radius = interaction.agent_radius_overrides[agent_participant_type]
                break
        try:
            ...
            if original_agent_radius is not None:
                routing_agent.routing_context.agent_radius = original_agent_radius
        ...
        if isinstance(dest, SlotGoal) and not dest_path[-1].body.posture_type.mobile:
        ...
        while cur_path.next_path is not None:
            ...
            if not left_path_spec.create_route_nodes(cur_path, portal_obj=portal_obj, portal_id=portal_id):
                ...
                return (False, failure_path, None)
            if portal_obj is not None:
                portal_instance = portal_obj.get_portal_by_id(portal_id)
                if portal_instance is not None and portal_instance.traversal_type.discourage_portal_on_plan:
        ...
        if left_path_spec._path[-1].posture_spec != right_path_spec._path[0].posture_spec:
            ...
            if right_first_spec.portal_obj is not None or sim.is_riding_horse:
    ...
    def check_and_build_generic_sim_carry_node(self, sim):
        if self._graph.proxy_sim is None and self._is_eligible_generic_carry_sim(sim):
            self.rebuild()
        ...
        try:
            ...
            if edge_info is None:
                if spec_a.body.posture_type != spec_b.body.posture_type or spec_a.carry != spec_b.carry:
                    ...
                    return EdgeInfo((), None, 0, 'No existing edge info')
            return edge_info
        except:
            pass
