...
class BaseObject(ComponentContainer):
    ...
    @property
    def non_deletable(self):
        return self._has_placement_flag(build_buy.PlacementFlags.NON_DELETEABLE)
    ...
    def cancel_interactions_running_on_object(self, finishing_type, cancel_reason_msg, interaction_to_skip=None):
        if self._interaction_refs is None:
            return
        for interaction in tuple(self._interaction_refs):
            if interaction not in self._interaction_refs:
                pass
            elif interaction is interaction_to_skip:
                pass
            else:
