...
class _FromParticipant(CreationDataBase, HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'participant': TunableEnumEntry(description='\n            The participant object we would like to create an instance of.\n            ', tunable_type=ParticipantType, default=ParticipantType.PickedObject)}

    def get_definition(self, resolver:'Resolver') -> 'Optional[Definition]':
        participant_object = self.get_participant(resolver)
        if participant_object is not None:
            return participant_object.definition

    def get_participant(self, resolver:'Resolver') -> 'Optional[Any]':
    ...
    def get_definition(self, resolver):
        if self.definition_override is not None:
            ...
            if source_object is not None:
                return source_object.default_definition
...
class _FishingDataFromParticipant(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'participant': TunableEnumEntry(description='\n            Participant on which we will get the fishing data information \n            ', tunable_type=ParticipantTypeObject, default=ParticipantTypeObject.Object), 'luck_config': OptionalTunable(description='\n            If enabled, Luck will influence which fish is selected.\n            ', tunable=LuckConfig.TunableFactory())}

    def get_fish_definition(self, resolver:'Resolver') -> 'Definition':
        ...
        if fishing_data is None:
            ...
            return
        fish_choice_result = fishing_data.choose_fish(resolver, require_bait=False, luck_config=self.luck_config)
        fish = fish_choice_result.chosen_item
        if fish is None:
            ...
            return
        if fish_choice_result.should_show_luck_impact:
            fish_choice_result.luck_level.impact_loot.apply_to_resolver(resolver)
...
class _FishingDataFromReference(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def get_fish_definition(self, resolver:'Resolver') -> 'Definition':
        ...
        if fishing_data is None:
            ...
            return
        fish_choice_result = self.fishing_data.choose_fish(resolver, require_bait=False, luck_config=self.luck_config)
        fish = fish_choice_result.chosen_item
        if fish is None:
...
class ObjectCreationMixin:
    ...
    def initialize_helper(self, resolver:'Resolver', post_add:'Optional[Callable[[GameObject], None]]'=None, creation_params:'Optional[ObjectCreationParams]'=None):

        def setup_and_post_add(obj:'GameObject') -> 'None':
            self._setup_created_object(obj)
        ...
        self._object_helper = CreateObjectHelper(reserved_sim, self._definition, interaction, object_to_clone=self.creation_data.get_source_object(self.resolver), post_add=setup_and_post_add)
    ...
    def _setup_created_object(self, created_object:'GameObject') -> 'None':
    ...
    def _place_object(self, created_object:'GameObject') -> 'bool':
