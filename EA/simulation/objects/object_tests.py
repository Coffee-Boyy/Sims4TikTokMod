...
class CraftedWithSkillFactory(TunableFactory):
    ...
    def __init__(self, **kwargs):
        super().__init__(skill_to_test=TunableReference(manager=services.get_instance_manager(sims4.resources.Types.STATISTIC), description='Skills needed to pass amount on.', pack_safe=True), description='This option tests for an item craft with the selected skill', **kwargs)
...
class CraftActualItemFactory(TunableFactory):
    ...
    def __init__(self, **kwargs):
        super().__init__(items_to_check=TunableList(TunableReference(manager=services.definition_manager(), description='Object that qualifies for this check.', pack_safe=True)), description='This option tests crafted item against a list of possible items', **kwargs)
...
class ObjectDefinitionCriteriaTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def __call__(self, target_objects=None):
        ...
        if self.fail_none_found and not target_objects:
            return TestResult(False, 'Target objects list was empty and Fail None Found checked.', tooltip=self.tooltip)
