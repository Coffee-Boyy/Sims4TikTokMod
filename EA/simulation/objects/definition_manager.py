...
class DefinitionManager(InstanceManager):
    ...
    def on_packs_loaded(self) -> None:
        super().on_packs_loaded()
        self._definitions_cache.clear()

    def get(self, def_id, obj_state=0, pack_safe=False, get_fallback_definition_id=True, class_restrictions=(), source=None):
        def_id = int(def_id)
        ...
        if definition is None:
            definition = self._load_definition_and_tuning(def_id, obj_state, pack_safe=pack_safe)
        self.verify_class_restrictions(class_restrictions, definition, source)
        return definition
