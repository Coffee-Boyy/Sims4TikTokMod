...
class AnimalService(Service):
    ...
    GARDENING_HELP_ANIMAL_STATES = TunableList(description="\n        For animals with gardening help enabled, these are the states that\n        an animal will transition from/to whenever the plant's state changes.\n        The order of these lists is the preference in the animal to pick.\n        ", tunable=TunableTuple(animal_type=TunableEnumEntry(tunable_type=CreatureType, default=CreatureType.Rabbit, invalid_enums=(CreatureType.Invalid,)), states=TunableList(tunable=TunableTuple(from_state=TunableReference(manager=services.get_instance_manager(sims4.resources.Types.OBJECT_STATE), class_restrictions=('ObjectStateValue',)), to_state=TunableReference(manager=services.get_instance_manager(sims4.resources.Types.OBJECT_STATE), class_restrictions=('ObjectStateValue',))))))
    ...
    @property
    def creature_aging_enabled(self):
        return self._creature_aging_enabled
    ...
    def get_animal_cost_obj(self, resolver, animal, use_curr_value=False):
        if animal is None:
            return 0
        animal_catalog_base_price = animal.current_value if use_curr_value else animal.definition.price
        catalog_price_modifier = animal.animalobject_component.catalog_price_modifier
        if catalog_price_modifier is None:
            return 0
        animal_cost_multiplier = catalog_price_modifier.multiplier
        animal_cost_tested_multiplier = catalog_price_modifier.tested_multipliers.get_multiplier(resolver)
        return int(animal_catalog_base_price*animal_cost_multiplier*animal_cost_tested_multiplier)
