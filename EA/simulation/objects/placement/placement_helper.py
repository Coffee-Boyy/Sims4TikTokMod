...
class _LocationFromSimFilter(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def get_objects_gen(self, resolver):
        ...
        for sim in services.sim_info_manager().instanced_sims_gen():
            if services.sim_filter_service().does_sim_match_filter(sim.id, requesting_sim_info=active_sim_info, sim_filter=self.sim_filter):
                yield sim
...
class _PlacementStrategyLocation(_PlacementStrategy):
    ...
    def _try_place_object_internal(self, obj:'GameObject', target_obj:'GameObject', resolver:'Resolver', ignored_object_ids:'Optional[Tuple[int, ...]]'=None, **kwargs) -> 'bool':
        if obj not in services.object_manager().values():
            return False
        ...
        if self.remove_from_parent_slot and obj.parent_slot is not None:
            obj.clear_parent(target_obj.transform, target_obj.routing_surface)
