...
class MannequinComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=MANNEQUIN_COMPONENT, persistence_key=SimObjectAttributes_pb2.PersistenceMaster.PersistableData.MannequinComponent):
    ...
    @distributor.fields.ComponentField(op=distributor.ops.PreloadSimOutfit, default=[])
    ...
    @distributor.fields.ComponentField(op=distributor.ops.SetGrubby, default=False)
    ...
    @distributor.fields.ComponentField(op=distributor.ops.SetMessy, default=False)
    ...
    @distributor.fields.ComponentField(op=distributor.ops.SetSinged, default=False)
    ...
    def on_state_changed(self, state, old_value, new_value, from_init):
        ...
        if self.state_trigger_singed is not None and self.state_trigger_grubby is not None and self.state_trigger_messy_face is not None and active_sim_info is not None:
            sim = active_sim_info.sim_info.get_sim_instance()
            if sim is not None:
                sim_inventory = sim.inventory_component
                sim_inventory.push_inventory_item_update_msg(self.owner)
