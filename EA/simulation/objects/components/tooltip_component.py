...
class TooltipComponent(Component, CustomTooltipTuningProvidingMixin, TooltipProvidingComponentMixin, HasTunableFactory, AutoFactoryInit, component_name=types.TOOLTIP_COMPONENT):
    ...
    def update_object_tooltip(self) -> 'None':
        if not self.hovertip_requested:
            return
        ...
        try:
            ...
            if self.tooltip_tests.run_tests(resolver):
                ...
                if tooltip_component._ui_metadata_handles:
                    subtext = tooltip_component.get_state_strings(subtext_fields, resolver)
    ...
    def get_state_strings(self, additional_strings:'Optional[List[str]]'=None, resolver:'Optional[Resolver]'=None) -> 'Optional[str]':
        obj = self.owner
        int_token = 0
        ...
        for state_string_datas in self.state_value_strings:
            for state_string_data in state_string_datas:
                if not state_string_data.state_value_tests.run_tests(resolver):
                    pass
                elif state_string_data.tooltip_field_override is not None:
                    pass
                else:
    ...
    @componentmethod_with_fallback(lambda : None)
    def get_state_strings_overrides(self) -> 'Dict[TooltipFields, str]':
        obj = self.owner
        int_token = 0
        for state_int_data in self.state_value_numbers:
            state_value = state_int_data.state_value
    ...
    def get_tooltip_field(self, field, context=None, target=None):
        ...
        if tooltip_text is not None:
            ...
            if tooltip_component._ui_metadata_handles:
                subtext = tooltip_component.get_state_strings(text, resolver=resolver)
                if subtext is not None:
                    text = subtext
