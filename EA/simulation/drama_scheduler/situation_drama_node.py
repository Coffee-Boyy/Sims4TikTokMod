...
class SituationDramaNode(BaseDramaNode):
    ...
    def _run_situation(self, duration_override:'Optional[TimeSpan]'=None, **extra_kwargs) -> 'None':
        host_sim_id = 0
        host_sim_info = None
        ...
        self.situation_id = services.get_zone_situation_manager().create_situation(self.situation_to_run, guest_list=guest_list, spawn_sims_during_zone_spin_up=self.spawn_sims_during_zone_spin_up, user_facing=self.user_facing, zone_id=zone_id, duration_override=duration_override, **extra_kwargs)
        if self.household_milestone and self.situation_id:
