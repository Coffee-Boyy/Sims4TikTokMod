...
class PlayerPlannedDramaNode(BaseDramaNode):
    ...
    def _run(self):
        ...
        if not self._validate_venue():
            ...
            zone_name = zone_data.name if zone_data is not None else ''
            notification.show_dialog(additional_tokens=(zone_name, self._situation_seed.situation_type.display_name))
            return DramaNodeRunOutcome.FAILURE
