...
class ArbElement(distributor.ops.ElementDistributionOpMixin, elements.SubclassableGeneratorElement):
    ...
    @contextmanager
    def _maybe_skip_posture_graph_rebuild_for_snap(self, event_data, obj):
        if not obj.is_valid_posture_graph_object:
            yield None
            return
        obj.should_rebuild_posture_graph_for_snap_event = event_data.event_data.get('snap_should_rebuild_posture_graph', True)
        try:
            yield None
        finally:
            obj.should_rebuild_posture_graph_for_snap_event = True
    ...
    def _event_handler_snap(self, event_data):
        ...
        if early_out is not None:
            return
        with self._maybe_skip_posture_graph_rebuild_for_snap(event_data, object_to_snap):
            ...
            if snap_actor_namespace is None:
                ...
                is_snap_to_ground = event_data.event_data['snap_ground']
                if is_snap_to_ground:
                    surface = object_to_snap.location.routing_surface
                    x = snap_transform.translation.x
                    z = snap_transform.translation.z
                    terrain_height = terrain.get_terrain_height(x, z, routing_surface=surface)
                    snap_transform = _math.Transform(_math.Vector3(x, terrain_height, z), snap_transform.orientation)
