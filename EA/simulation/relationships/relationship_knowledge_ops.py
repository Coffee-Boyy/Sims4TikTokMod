...
class KnowOtherSimTraitOp(RelationshipBitTargetedLootOperation):
    ...
    def _build_trait_list_string(self, traits:'List[Trait]', target) -> 'str':
        ...
        if not self.notification_group_preferences:
            trait_names = []
            for trait in traits:
                if len(trait.display_overrides):
                    trait_names.append(trait.display_overrides[0].display_name_override(target))
                else:
                    trait_names.append(trait.display_name(target))
            return LocalizationHelperTuning.get_bulleted_list((None,), trait_names)
        default_traits = []
