...
class StoryProgressionService(Service):
    ...
    def _seed_arcs_from_pass_and_neighborhood_gen(self, pass_tuning, neighborhood_proto, timeline, gsi_data, debug_seed_all_arcs=False):
        ...
        for i in range(number_to_seed):
            ...
            elif potential_arc.arc_type == StoryType.MULTI_SIM_BASED and sim_candidate is not None and len(sim_candidate) > 1:
                story_progression_tracker = sim_candidate[0].story_progression_tracker
                chosen_candidate = sim_candidate[0]
            ...
            if story_progression_tracker is None:
                ...
                if not result:
                    ...
                    if potential_arc.arc_type == StoryType.MULTI_SIM_BASED:
                        arc = story_progression_tracker._current_arcs[-1]
                        arc.store_participant(participant=ParticipantType.SavedStoryProgressionSim1, obj=sim_candidate[1].id)
                        if len(sim_candidate) > 2:
                            arc.store_participant(participant=ParticipantType.SavedStoryProgressionSim2, obj=sim_candidate[2].id)
                    ...
                    elif potential_arc.arc_type == StoryType.MULTI_SIM_BASED:
                        arc_gsi_data.item_id = sim_candidate[0].id
                        arc_gsi_data.item_name = sim_candidate[0].full_name
                    else:
                    ...
                    if potential_arc.arc_type == StoryType.SIM_BASED:
                        ...
                        if chosen_candidate.id not in self._sim_story_progression_trackers:
                            self._sim_story_progression_trackers.add(chosen_candidate.id)
                        for candidate in sim_candidate:
                            if candidate in sim_infos:
                                sim_infos.remove(candidate)
                    else:
    ...
    def _clear_chapter_history(self, chapter):
        ...
        if tracker.historical_arcs:
            return
        if tracker.tracker_type & StoryType.SIM_BASED or tracker.tracker_type & StoryType.MULTI_SIM_BASED:
            self._historical_sim_story_progression_trackers.remove(tracker.sim_info.id)
        elif tracker.tracker_type & StoryType.HOUSEHOLD_BASED:
