...
class RoommateService(Service):
    ...
    ROOMMATE_RELATIONSHIP_BIT = TunablePackSafeReference(description='\n        The relationship bit to add between sims in the household that owns the\n        lot, and any roommates\n        ', manager=services.get_instance_manager(sims4.resources.Types.RELATIONSHIP_BIT), class_restrictions=('RelationshipBit',), verify_tunable_callback=_verify_tunable_callback)
    ROOMMATE_SIM_FILTERS = TunableList(description='\n        List of paired tests and simfilters.\n        The first simfilter whose test passes will be used to acquire (potential)\n        roommates.\n        ', tunable=TunableTuple(test=TunableTestSet(), sim_filter=TunableReference(description='\n                Sim Filter to find roommates if the associated test is the first\n                one to pass.\n                ', manager=services.get_instance_manager(sims4.resources.Types.SIM_FILTER), class_restrictions=('TunableSimFilter',), pack_safe=True)))
    ...
    ROOMMATE_JOB = TunablePackSafeReference(description='\n        The job for the roommate\n        ', manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB), class_restrictions=('SituationJob',), verify_tunable_callback=_verify_tunable_callback)
    ...
    SS3_PARK_INTERACTIONS = TunableList(description='\n        Interactions in which to park roommates during SS3\n        ', tunable=TunableReference(manager=services.get_instance_manager(sims4.resources.Types.INTERACTION), class_restrictions=('SuperInteraction',), pack_safe=True))
    ...
    def _get_household_from_zone_id(self, zone_id):
        ...
        if zone_id == current_zone.id:
            zone_owner_hh_id = current_zone.lot.zone_owner_household_id
            if zone_owner_hh_id != household_id:
                logger.warn('Attempting to retrieve the household on zone: {}, but there is a mismatch between the cached proto buff (hh_id: {}) and the queried native lot data (hh_id: {}).', zone_id, household_id, zone_owner_hh_id)
                return
            elif household_id:
