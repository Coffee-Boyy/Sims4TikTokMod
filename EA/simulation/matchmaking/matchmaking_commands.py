...
@sims4.commands.Command('matchmaking.get_contacted_sim_ids', pack=Pack.EP16, command_type=sims4.commands.CommandType.Automation)
def get_contacted_sim_ids(actor_param:RequiredTargetParam, _connection:int=None) -> None:
    matchmaking_service = _get_matchmaking_service(_connection)
    if matchmaking_service is None:
        sims4.commands.automation_output('ContactedSimIds; Ids:', _connection)
        return
    actor_info = actor_param.get_target(manager=services.sim_info_manager())
    if actor_info is None:
        sims4.commands.automation_output('ContactedSimIds; Ids:', _connection)
    ...
    if actor_info.id in matchmaking_service.actor_id_to_matchmaking_data:
        sim_ids = set()
        for candidate_id in matchmaking_service.actor_id_to_matchmaking_data[actor_info.id].candidate_ids:
            matchmaking_profile = matchmaking_service.candidate_id_to_matchmaking_profile[candidate_id]
            if matchmaking_profile.contacted:
                sim_ids.add(matchmaking_profile.real_sim_id)
        id_string = str(sim_ids)
        id_string = id_string.translate({ord(','): None, ord('}'): None, ord('{'): None})
        sims4.commands.automation_output('ContactedSimIds; Ids:{}'.format(id_string), _connection)
        return
    sims4.commands.automation_output('ContactedSimIds; Ids:', _connection)
    sims4.commands.output('Actor ({}) not found in matchmaking data'.format(actor_info), _connection)
