...
class FindGoodLocationContext:
    ...
    def find_good_location(self):
        ...
        if search_result == FGLSearchResult.SUCCESS:
            ...
            result_message = 'FGL Failed: {0}.'.format(str(search_result))
...
def create_fgl_context_for_sim(starting_location:'Location', sim_to_place:'Union[SimInfo, Sim]', search_flags:'FGLSearchFlag'=FGLSearchFlagsDefaultForSim, water_depth_info:'WaterDepthInfo'=None, can_sim_swim_override:'Optional[bool]'=None, **kwargs) -> 'FindGoodLocationContext':
    object_id = None
    if search_flags & FGLSearchFlag.SHOULD_TEST_BUILDBUY:
        object_id = sim_to_place.id
    if sim_to_place.sim_info is not sim_to_place:
        routing_context = sim_to_place.routing_context
    else:
        logger.error('create_fgl_context_for_sim called for SimInfo {}. It should be only called for sim objects. Please check the code.', sim_to_place)
        routing_context = None
    if water_depth_info is None:
        if can_sim_swim_override is None:
            can_sim_swim = sim_to_place.can_swim()
        else:
            can_sim_swim = can_sim_swim_override
        if not can_sim_swim:
            water_depth_info = WaterDepthInfo(max_water_depth=0.0)
    return FindGoodLocationContext(starting_location, object_id=object_id, routing_context=routing_context, search_flags=search_flags, water_depth_info=water_depth_info, **kwargs)
