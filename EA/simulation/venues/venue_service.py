...
class VenueService(Service):
    ...
    def __init__(self):
        ...
        self._venue_business_data_proto = None
    ...
    @property
    def venue_business_data_proto(self) -> GameplaySaveData_pb2.VenueBusinessData:
        return self._venue_business_data_proto
    ...
    def _send_venue_time_spent_telemetry(self):
        ...
        if time_spent_mins:
            with telemetry_helper.begin_hook(venue_telemetry_writer, TELEMETRY_HOOK_TIMESPENT) as hook:
                ...
                lot_description_id = services.get_lot_description_id(services.active_lot_id(), services.get_world_description_id(services.current_zone().world_id))
                hook.write_int(TELEMETRY_FIELD_LOT_ID, lot_description_id)
    ...
    def request_zone_director(self, zone_director, request_type, preserve_state=True):
        ...
        for (prior_request_type, prior_zone_director, prior_preserve_state) in self._requested_zone_directors:
            if prior_request_type == request_type:
                self._requested_zone_directors.remove((prior_request_type, prior_zone_director, prior_preserve_state))
    ...
    def on_cleanup_zone_objects(self, client):
        zone = services.current_zone()
        if client.household_id != zone.lot.zone_owner_household_id:
    ...
    def send_business_data_update_message(self, business_data:GameplaySaveData_pb2.VenueBusinessData, zone_id:int) -> None:
        persistence_service = services.get_persistence_service()
        zone_data = persistence_service.get_zone_proto_buff(zone_id)
        if zone_data is not None:
            venue_data = GameplaySaveData_pb2.VenueData()
            venue_data.CopyFrom(zone_data.gameplay_zone_data.venue_data)
            venue_data.business_data = business_data
            distributor = Distributor.instance()
            venue_data_update_msg = GameplaySaveData_pb2.VenueDataUpdate()
            venue_data_update_msg.zone_id = zone_id
            venue_data_update_msg.venue_data = venue_data
            distributor.add_event(Consts_pb2.MSG_CACHE_UPDATE_VENUE, venue_data_update_msg)
        else:
            logger.error('Trying to send business data update to client, but business data is None.')
