...
class WeeklyScheduleSituationData(metaclass=sims4.tuning.instances.HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SNIPPET)):
    ...
    @classmethod
    def try_start(cls, sims:'Set[HashedTunedInstanceMetaclass]', sim_info:'SimInfo', duration:'TimeSpan', additional_sim_ids:'Set[int]', situation_job_count:'defaultdict') -> 'Optional[int]':
...
class WeeklyScheduleDay(metaclass=sims4.tuning.instances.HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SNIPPET)):
    ...
    @classmethod
    def start_situations(cls, situation_sets:'tuple', resolver:'Resolver', sim_info:'SimInfo', duration:'TimeSpan', additional_sim_ids:'Set[int]', on_time:'bool', delayed_loots:'List'=None) -> 'List[int]':
    ...
    def start_long_term_situation(cls, situation, start_time, stop_time, min_count, max_count, requesting_sim_info, situation_ids):
        ...
        duration = now.time_till_next_day_time(stop_time, rollover_same_time=True)
    ...
    def request_initial_long_term_situations(cls, resolver, requesting_sim_info, existing_ids):
        ...
        while existing_situations:
            ...
            if situation_times is None:
                ...
                situation.change_duration_by_timespan(now.time_till_next_day_time(closest_time))
...
class WeeklyScheduleZoneDirector(CareerEventZoneDirectorMixin, ZoneDirectorBase):
    ...
    def _on_scheduled_situation_request(self, *_, delayed_loots=None, **__):
        ...
        (self._situation_ids, self._situation_set) = self._schedule.start_situations(situation_sets, self._get_resolver(), self._get_relevant_sim_info(), time_span, set(career_event.sim_info.sim_id for career_event in self._career_events if career_event.sim_info is not None), on_time, delayed_loots=delayed_loots)
