...
class ResidentialTravelDisplayName(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def __call__(self, target, context):
        ...
        household_id = None
        zone_data = persistence_service.get_zone_proto_buff(to_zone_id)
...
class Venue(metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.VENUE)):
    ...
    def _try_start_special_event(self, scheduler, alarm_data, extra_data):
        ...
        if self._active_special_event_id is None:
            ...
            if situation.venue_invitation_message is not None and (situation_manager.is_incompatible_user_facing_situation_running() or active_sim_available):
    ...
    def is_residential(cls):
        return cls.venue_type == VenueTypes.RESIDENTIAL or cls.venue_type == VenueTypes.MULTI_UNIT

    @classproperty
    def is_multi_unit(cls):
        return cls.venue_type == VenueTypes.MULTI_UNIT
    ...
    @classproperty
    def is_small_business(cls) -> bool:
        return cls.venue_type == VenueTypes.SMALL_BUSINESS
