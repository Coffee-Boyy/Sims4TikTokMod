...
@sims4.commands.Command('tuning.dump_tuning_dependencies_cache')
def dump_tuning_dependencies_cache(_connection=None):
    td_cache = {}
    td_cache.update(read_td_cache_from_resource())
    file_name = 'tuning_dependencies_cache_{}.txt'.format(time.strftime('%y%m%d_%H_%M_%S'))
    with open(file_name, 'w') as fd:
        for (dest, sources) in td_cache.items():
            (dest_instance, dest_type) = dest
            for source in sources:
                (source_instance, source_type) = source
                fd.write('({},{})\t<-\t({},{})\n'.format(dest_instance, dest_type, source_instance, source_type))
    output_str = "Dumped tuning dependencies info to: '{}'".format(file_name)
    sims4.commands.output(output_str, _connection)
    return True

@sims4.commands.Command('tuning.load_pack', command_type=sims4.commands.CommandType.Automation)
def load_pack(pack:Pack=None, _connection:int=None) -> bool:
    ...
    result = InstanceTuningManagers.hot_load_packs((pack,), log_fn=_log_output)
    return result

@sims4.commands.Command('tuning.unload_pack', command_type=sims4.commands.CommandType.Automation)
def unload_pack(pack:Pack=None, _connection:int=None) -> bool:
    from services.tuning_managers import InstanceTuningManagers
    result = InstanceTuningManagers.hot_unload_packs((pack,))
    return result
