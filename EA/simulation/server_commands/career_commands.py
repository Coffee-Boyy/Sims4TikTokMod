...
@sims4.commands.Command('careers.make_sim_event_temp')
def career_make_sim_event_temp(opt_sim:OptionalSimInfoParam=None, _connection=None):
    sim_info = get_optional_target(opt_sim, target_type=OptionalSimInfoParam, _connection=_connection)
    if sim_info is None:
        return False
    career = services.get_career_service().get_career_in_career_event()
    ...
    if career_event is None:
        return False
    career_event.make_sim_temporary(sim_info)
    return True
