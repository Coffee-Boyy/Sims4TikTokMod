...
@sims4.commands.Command('situations.activate_minimized_situation', command_type=sims4.commands.CommandType.Live)
def activate_minimized_situation(pivotal_moment_id:int, _connection:int=None) -> None:
    tutorial_manager = services.get_tutorial_service()
    if tutorial_manager:
        tutorial_manager.activate_pivotal_moment(pivotal_moment_id)

@sims4.commands.Command('situations.activate_minimized_live_events', command_type=sims4.commands.CommandType.Live)
def activate_minimized_live_events(_connection:int=None) -> None:
    tutorial_manager = services.get_tutorial_service()
    if tutorial_manager:
        tutorial_manager.activate_minimized_live_events()

@sims4.commands.Command('situations.reevaluate_minimized_situations', command_type=sims4.commands.CommandType.Live)
def reevaluate_minimized_situations(_connection:int=None) -> None:
    tutorial_manager = services.get_tutorial_service()
    if tutorial_manager:
        tutorial_manager.reevaluate_pending_situations()
...
@sims4.commands.Command('situations.enable_minimized_situations', command_type=sims4.commands.CommandType.DebugOnly)
def enable_minimized_situations(enabled:bool=True, _connection=None):
    tutorial_manager = services.get_tutorial_service()
    if tutorial_manager is not None:
        if enabled:
            tutorial_manager.set_minimized_situations_enabled(True)
        else:
            tutorial_manager.set_minimized_situations_enabled(False)
