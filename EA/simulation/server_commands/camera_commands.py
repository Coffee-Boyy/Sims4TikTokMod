...
@sims4.commands.Command('camera.focus_on_sim', command_type=sims4.commands.CommandType.Live)
def focus_on_sim(sim_info:SimInfoParam, *_, _connection=None, **__):
    ...
    if sim is None:
        sims4.commands.output('Could not find Sim to focus on.', _connection)
        return
    camera.focus_on_sim(sim, True)
    sims4.commands.output('focus on sim: {}'.format(sim_info), _connection)

@sims4.commands.Command('camera.focus_on_locator', command_type=sims4.commands.CommandType.Live)
