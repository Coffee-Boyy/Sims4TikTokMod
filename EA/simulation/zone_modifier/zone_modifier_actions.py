    ...
    def _perform_action(self):
        ...
        if distribution_type == self.ACTIVE_SIM_ONLY:
            ...
            if active_sim is None:
                ...
                if zone_spin_up_service._client_connect_data is None:
                    return
                active_sim_id = zone_spin_up_service._client_connect_data.active_sim_id
                active_sim = services.sim_info_manager().get(active_sim_id)
                if active_sim is None or not active_sim.get_is_enabled_in_skewer(consider_active_sim=False):
                    active_sim = services.get_selectable_sims().get_next_selectable(active_sim)
                    if active_sim is None:
                        return
