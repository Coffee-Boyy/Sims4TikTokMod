...
class BusinessService(Service):
    ...
    def __init__(self) -> 'None':
        ...
        self._cached_rental_unit_data_for_propery_owner_removal = {}

    def remove_cached_rental_unit_data(self, zone_id):
        del self._cached_rental_unit_data_for_propery_owner_removal[zone_id]

    def get_cached_rental_unit_data(self, zone_id) -> 'BusinessManager':
        if not self._cached_rental_unit_data_for_propery_owner_removal:
            return
        return self._cached_rental_unit_data_for_propery_owner_removal[zone_id]
    ...
    def remove_owner(self, zone_id, household_id=None, unowned_business_type:'int'=BusinessType.INVALID):
        ...
        if business_tracker is None:
            return
        business_manager = business_tracker.remove_owner(zone_id)
        if business_manager.is_open:
            self._cached_rental_unit_data_for_propery_owner_removal[zone_id] = business_manager
        if business_manager.business_type == BusinessType.RENTAL_UNIT and unowned_business_type != BusinessType.INVALID:
    ...
    def save(self, save_slot_data:'SaveSlotData'=None, **kwargs) -> 'None':
        ...
        if hasattr(business_service_data, 'rule_presets_data'):
            for (name, rules) in self.rules_presets.items():
