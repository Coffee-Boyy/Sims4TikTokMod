...
class EcoFootprintStateProviderMixin(ComponentContainer, HasStatisticComponent):
    ...
    def finalize_startup(self):
        ...
        sun_ray_objects = list(services.object_manager().get_objects_matching_tags((EcoFootprintTunables.SUN_RAY_TAG,), match_any=True))
        if len(sun_ray_objects) > 1:
            for i in range(len(sun_ray_objects) - 1):
                obj = sun_ray_objects[i]
                if obj.is_sim:
                    pass
                else:
                    obj.destroy(cause='Remove extra sun ray objects')
