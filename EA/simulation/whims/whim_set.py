...
class WhimSetBaseMixin:
    ...
    @blueprintmethod
    def get_priority(self, sim_info:'SimInfo'):
        raise NotImplementedError

    @blueprintmethod
    def activate(self, whims_tracker, chained, target):
        pass

    @blueprintproperty
    def found_whim_types(self):
        whim_types = set()
        for entry in self.whims:
...
class AspirationWhimSet(WhimSetBaseMixin, AspirationBasic):
    ...
    @blueprintmethod
    def _tuning_loaded_callback(self) -> 'None':
        commodity = RuntimeCommodity.generate(self.tuning_name)
        commodity.decay_rate = self.priority_decay_rate
        commodity.convergence_value = 0
        commodity.remove_on_convergence = False
        commodity.visible = False
        if self.activated_priority > self.chained_priority:
            commodity.max_value_tuning = self.activated_priority
        else:
            commodity.max_value_tuning = self.chained_priority
        ...
        self.priority_commodity = commodity

    @blueprintmethod
    def _verify_tuning_callback(self) -> 'None':
        if self.activated_priority == 0 and self.chained_priority == 0:
            logger.error('No priority tuned for value greater than 0 in {}', self)
        for objective in self.objectives:
            if objective.objective_completion_type == objective_tuning.SimInfoStatisticObjectiveTrack:
                logger.error("{} Objective in {} Whim Set tuned with incorrect Objective test type; use 'iterations', 'unique_locations', or 'unique targets'.", objective, self)
            if not objective.resettable:
                logger.error('{} Objective in {} Whim Set tuned as a Whim Aspiration Objective but not tuned as resettable. All Aspriation Whim Set objectives must be resettable.', objective, self)
        if self.timeout_retest is not None and (self.timeout_retest.objective_test.USES_EVENT_DATA or self.timeout_retest.objective_test.USES_DATA_OBJECT):
            logger.error('Bad Tuning! {} Objective Test {} in Whim Set being used as a timeout_retest cannot use event or object data.', self.timeout_retest.objective_test, self)

    @blueprintmethod
    def get_priority(self, sim_info:'SimInfo') -> 'float':
        whimset_priority_stat = sim_info.get_statistic(self.priority_commodity, add=False)
...
class ObjectivelessWhimSet(WhimSetBaseMixin, AspirationBasic):
    ...
    @blueprintproperty
    def deactivate_on_completion(self) -> 'bool':
        return False
