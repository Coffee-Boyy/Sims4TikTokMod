...
class UniversityUtils:
    ...
    @staticmethod
    def _get_household_sims_to_kickout(household:'Household', kickout_payloads:'List[KickoutPayload]') -> 'None':
        for sim_info in household.sim_info_gen():
            if not sim_info is None:
                if sim_info.degree_tracker is None:
                    ...
                    if kick_out_reason == UniversityHousingKickOutReason.NOT_ENROLLED:
                        venue_manager = services.get_instance_manager(sims4.resources.Types.VENUE)
                        venue = venue_manager.get(build_buy.get_current_venue(sim_info.household.home_zone_id))
                        if sim_info.degree_tracker.kickout_destination_zone == 0:
                            destination_zone_id = sim_info.household.home_zone_id
