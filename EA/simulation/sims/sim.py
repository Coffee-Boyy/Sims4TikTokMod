...
class Sim(HasSimInfoMixin, LockoutMixin, EnvironmentScoreMixin, GameObject):
    ...
    _reaction_triggers = {}
    FOREIGN_ZONE_BUFF = TunableReference(description='\n        This buff is applied to any sim that is not in their home zone.  It is\n        used by autonomy for NPCs to score the GoHome interaction.\n        ', manager=services.get_instance_manager(sims4.resources.Types.BUFF), class_restrictions=('Buff',))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.waiting_for_carry_fixup = False
    ...
    @property
    def in_fairy_form(self) -> 'bool':
        sim_info = self.sim_info
        if self.species != SpeciesExtended.HUMAN or sim_info.occult_tracker is None or not sim_info.occult_tracker.in_occult_form(OccultType.FAIRY):
            return False
        return True
    ...
    def update_intended_position_on_active_lot(self, *_, update_ui=False, **__):
        ...
        parent = self.parent
        if parent.species is not Species.HORSE:
            on_active_lot = True
        ...
        if parent is not None and parent.is_sim and on_off_lot_update or update_ui:
    ...
    def get_delete_op(self, fade_duration:'float'=0):
        services.sim_spawner_service().add_sim_cooldown(self, math.ceil(fade_duration/60))
        return super().get_delete_op(fade_duration=fade_duration)
    ...
    def _on_navmesh_updated(self):
        ...
        if self.transition_controller is not None:
            if self.routing_component.current_path is not None and self.routing_component.current_path.nodes.needs_replan():
                if self.transition_controller.succeeded and not self.transition_controller.interaction.waypoint_count > 0:
    ...
    def validate_location(self, location, from_reset=False, from_spawn=False):
        ...
        if routing_location.routing_surface.type == routing.SurfaceType.SURFACETYPE_POOL:
            ...
            return False
        if from_spawn:
            contexts = {self.routing_context}
        else:
            allowed_targets = set()

            def get_valid_saved_participants(interaction:'Interaction') -> 'Set[Any]':
                valid_participants = set()
                for participant in interaction.get_saved_participants():
                    if hasattr(participant, 'raycast_context'):
                        valid_participants.add(participant)
                    elif getattr(participant, 'is_sim', False) and participant == participant.sim_info:
                        valid_participants.add(participant.get_sim_instance())
            ...
            for interaction in itertools.chain((self.queue.running,), self.si_state):
                if interaction is None:
                    ...
                    allowed_targets.update(get_valid_saved_participants(interaction))
            if self.transition_controller is not None:
        ...
        if from_spawn:
            test_portal_clearance = False
        else:
            test_portal_clearance = self.posture.unconstrained and (not self.in_pool and not on_object_surface)
        if from_reset or from_spawn or not (on_object_surface and self.posture.unconstrained):
            agent_radius = routing.FAKE_AGENT_RADIUS_FOR_OBJECT_ROUTING_SURFACE_VALIDATION
        else:
            agent_radius = self.object_radius
        if from_reset:
            quadtree_polygon = self.routing_context.get_quadtree_polygon()
            blocking_sims = services.sim_quadtree().query(bounds=quadtree_polygon, surface_id=self.routing_surface, filter=placement.ItemType.SIM_POSITION, exclude=self.id)
            if blocking_sims:
                return False
    ...
    def validate_current_location_or_fgl(self, from_reset=False, from_spawn=False):
        parent = self.parent
        ...
        if not (self.can_swim() or self.should_be_swimming_at_position(self.position, self.location.level, check_can_swim=False)):
            if self.routing_surface.type == SurfaceType.SURFACETYPE_OBJECT:
                ...
                return
        if (zone.is_in_build_buy or (from_reset or from_spawn) or from_reset) and zone.is_in_build_buy:
        ...
        if self.routing_component.current_path is not None:
            if from_reset:
                return
            if self.intended_routing_surface.type == SurfaceType.SURFACETYPE_POOL and self.in_pool and self.routing_component.current_path.nodes.needs_replan():
                self.reset(ResetReason.RESET_EXPECTED, self, 'Routing surface or navmesh changed while Sim was swimming.')
        ...
        if self.validate_location(location, from_reset=from_reset, from_spawn=from_spawn):
            if not on_surface:
                if not from_reset:
        ...
        if trans is None or orient is None:
            if from_spawn:
                logger.error('Failed to find good location to spawn sim. They will still spawn in, but they will likely clip with things.')
                return
            if not from_reset:
            ...
            return
        if from_reset or not from_spawn:
            self.reset(ResetReason.RESET_EXPECTED, self, 'Failed to find location.')
    ...
    def on_reset_internal_state(self, reset_reason):
        ...
        try:
            if not being_destroyed:
                ...
                if self.zone_id != services.current_zone_id() and (posture_target is not None and posture_target.is_sim) and posture_target.species == Species.HORSE:
                    return
                if self.zone_id != services.current_zone_id() and self.carry_despawn:
            ...
            for interaction in self.si_state.sis_actor_gen():
                if not interaction.joinable or not interaction.is_finishing:
                    for aop in get_join_aops_gen(interaction, self, context.sim, create_join_si):
                        yield aop
                elif interaction.joinable_proxy and not interaction.is_finishing:
                    for linked_interaction in interaction.interactions:
                        if linked_interaction.joinable:
                            for aop in get_join_aops_gen(linked_interaction, self, context.sim, create_join_si):
                                yield aop
    ...
    def _potential_behavior_affordances_gen(self, context, **kwargs):
        ...
        for affordance in self.sim_info.whim_tracker.get_cached_super_affordances_gen():
            if self._can_show_affordance(shift_held, affordance):
                yield affordance
        if context is not None and self.sim_info.unlock_tracker is not None and self.sim_info.career_tracker is not None and self.sim_info.whim_tracker is not None and self.sim_info.aspiration_tracker is not None:
            for affordance in self.sim_info.aspiration_tracker.get_cached_super_affordances_gen():
                if self._can_show_affordance(shift_held, affordance):
                    yield affordance
    ...
    def on_add(self):
        ...
        if zone.is_zone_running:
            zone.posture_graph_service.check_and_build_generic_sim_carry_node(self)
    ...
    def _startup_sim_gen(self, timeline):
        ...
        try:
            ...
            if self._simulation_state == SimulationState.INITIALIZING:
                ...
                self.trait_tracker.remove_ungranted_traits()
                self.trait_tracker.ungranted_traits.clear()
                ...
                if self.sim_info.is_ghost:
                    self.sim_info.send_ghost_colors()
                    stat = self._sim_info.commodity_tracker.get_statistic(GhostPowersTunables.GHOST_POWERS_RANKED_STATISTIC)
                    if stat is not None:
                ...
                if services.current_zone().is_zone_running:
                    ...
                    if self.is_selectable:
                        ...
                        if open_street_director is not None and open_street_director.reactionary_whim_set is not None:
                            open_street_director.register_sim_for_reactionary_whim_callback(self.sim_info.get_sim_instance())
                if self.is_selected:
            ...
            self.on_outfit_changed(self.sim_info, self._sim_info.get_current_outfit(), None)
            if self.species == SpeciesExtended.HORSE:
                self._refresh_horse_tooltip_info()
                self.Buffs.on_mood_changed.append(self._on_mood_changed)
            self.refresh_los_constraint()
            ...
            jewelry_tracker = self.sim_info.jewelry_tracker
            if jewelry_tracker is not None:
                jewelry_tracker.on_sim_startup()
            tattoo_tracker = self.sim_info.tattoo_tracker
            if tattoo_tracker is not None:
                tattoo_tracker.on_sim_startup()
            family_recipes_tracker = self.sim_info.family_recipes_tracker
            if family_recipes_tracker is not None:
                family_recipes_tracker.apply_retroactive_buff_unlocks()
        finally:
            self._starting_up = False
            if previous_simulation_state == SimulationState.RESETTING:
                services.current_zone().service_manager.on_sim_reset(self)

    def update_horse_tooltip_info(self):
        self._refresh_horse_tooltip_info()

    def _refresh_horse_tooltip_info(self):
        self.update_tooltip_field(TooltipFieldsComplete.recipe_name, LocalizationHelperTuning.get_sim_full_name(self))
        aging_data = AgingTuning.get_aging_data(self.species)
        if aging_data is None:
            return
        current_age_data = aging_data.ages.get(self.age)
        gender_string = self.sim_info.get_gender_string_key()
        gender_symbol = self.sim_info.get_gender_symbol_key()
        if gender_string is not None:
            gender_text = LocalizationHelperTuning.get_separated_string_by_style(ConcatenationStyle.CONCATENATE_SEPARATION, gender_string, gender_symbol)
        else:
            gender_text = gender_symbol
        age_gender_text = LocalizationHelperTuning.get_name_value_pair(current_age_data.cas_name(self.sim_info), gender_text, comma_override=True)
        self.update_tooltip_field(TooltipFieldsComplete.header_subtext, age_gender_text, should_update=True)
        mood = self.get_mood()
        mood_intensity = self.get_mood_intensity()
        mood_name = _create_localized_string(mood.mood_names[mood_intensity].hash, self.sim_info)
        mood_text = LocalizationHelperTuning.get_colored_text(mood.mood_text_color, mood_name)
        self.update_tooltip_field(TooltipFieldsComplete.header_status, mood_text, should_update=True)
        self.update_tooltip_field(TooltipFieldsComplete.header, self.sim_info.sim_headline, should_update=True)
    ...
    def on_remove(self):
        ...
        if self.species == SpeciesExtended.HORSE and self._on_mood_changed in self.Buffs.on_mood_changed:
            self.Buffs.on_mood_changed.remove(self._on_mood_changed)
        if self.is_human:
            for static_commodity_type in self.sim_info.STATIC_COMMODITIES_WHILE_INSTANCED:
        ...
        for carry_target in value.carry_targets:
            if carry_target is not None:
                if value.get_carry_track(carry_target) != PostureTrack.BACK:
                    ...
                    front_carry_target = carry_target
                    front_key_mask = front_key_mask & ~carry_target.get_portal_key_make_for_carry()
                else:
                    back_carry_target_found = True
                    back_carry_target = carry_target
                    back_key_mask = back_key_mask & ~carry_target.get_portal_key_make_for_carry()
        flying_fairy_sim = self.has_trait(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.flying_walkstyle_trait)
        fairy_front_carry_applies = self._posture_state is not None and flying_fairy_sim and CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.apply_to_front_carries
        fairy_back_carry_applies = flying_fairy_sim and CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.apply_to_back_carries
        if front_carry_target_found:
            self.routing_component.clear_portal_mask_flag(front_key_mask)
            if fairy_front_carry_applies and front_carry_target.is_sim:
                self.set_fairy_sim_carrying_portal_flags(True)
        else:
            self.routing_component.set_portal_mask_flag(front_key_mask)
            if not (fairy_front_carry_applies and back_carry_target_found and back_carry_target.is_sim):
                self.set_fairy_sim_carrying_portal_flags(False)
        if back_carry_target_found:
            self.routing_component.clear_portal_mask_flag(back_key_mask)
            if fairy_back_carry_applies and back_carry_target.is_sim:
                self.set_fairy_sim_carrying_portal_flags(True)
        else:
            self.routing_component.set_portal_mask_flag(back_key_mask)
            if not (fairy_back_carry_applies and front_carry_target_found and front_carry_target.is_sim):
                self.set_fairy_sim_carrying_portal_flags(False)
        self._posture_target_refs.clear()
        for aspect in self._posture_state.aspects:
    ...
    def set_fairy_sim_carrying_portal_flags(self, set_flags=True):
        if set_flags:
            if CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags is not None:
                self.routing_component.set_portal_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags.portal_allowance_flags)
                self.routing_component.set_portal_discouragement_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags.portal_discouragement_flags)
                self.routing_component.set_portal_exclusion_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags.portal_exclusion_flags)
            if CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags is not None:
                self.routing_component.clear_portal_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags.portal_allowance_flags)
                self.routing_component.clear_portal_discouragement_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags.portal_discouragement_flags)
                self.routing_component.clear_portal_exclusion_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags.portal_exclusion_flags)
        else:
            if CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags is not None:
                self.routing_component.clear_portal_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags.portal_allowance_flags)
                self.routing_component.clear_portal_discouragement_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags.portal_discouragement_flags)
                self.routing_component.clear_portal_exclusion_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.set_flags.portal_exclusion_flags)
            if CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags is not None:
                self.routing_component.set_portal_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags.portal_allowance_flags)
                self.routing_component.set_portal_discouragement_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags.portal_discouragement_flags)
                self.routing_component.set_portal_exclusion_mask_flag(CarryTuning.FAIRY_SIM_CARRYING_PORTAL_FLAGS.unset_flags.portal_exclusion_flags)
    ...
    def set_trait_asm_parameters(self, asm, actor_name):
        ...
        asm_param_log = None
        locked_params_log_enabled = asm.locked_params_log_enabled
        if locked_params_log_enabled:
            param_context = LockedParamsContext(LockedParamCategory.SIM_TRAIT, str(self.sim_info), 'Default trait params')
            asm_param_log = create_locked_params_log(asm_param_dict, param_context)
        for (param_name, param_value) in self._default_anim_params.items():
            asm_param_dict[(param_name, actor_name)] = param_value
            if locked_params_log_enabled:
                param_context = LockedParamsContext(LockedParamCategory.SIM_ANIM, str(self.sim_info), 'Default anim params', param_value)
                asm_param_log = add_context_to_locked_params_log(asm_param_log, (param_name, actor_name), param_context)
        for trait in self.sim_info.get_traits():
            if trait.trait_asm_overrides.trait_asm_param is None:
                ...
                if trait.trait_asm_overrides.param_type is None:
                    asm_key = (trait.trait_asm_overrides.trait_asm_param, actor_name)
                    asm_value = True
                    asm_param_dict[asm_key] = asm_value
                else:
                    asm_key = (trait.trait_asm_overrides.param_type, actor_name)
                    asm_value = trait.trait_asm_overrides.trait_asm_param
                    asm_param_dict[asm_key] = asm_value
                if locked_params_log_enabled:
                    param_context = LockedParamsContext(LockedParamCategory.SIM_TRAIT, str(self.sim_info), 'ASM override from sim trait ' + str(trait), asm_value)
                    asm_param_log = add_context_to_locked_params_log(asm_param_log, asm_key, param_context)
        asm.update_locked_params(asm_param_dict, ignore_virtual_suffix=True, locked_params_log=asm_param_log)
    ...
    def get_next_work(self):
        ...
        next_interaction = self.queue.get_head()
        if not self.waiting_for_carry_fixup:
            if any(not i.is_super for i in self.queue._autonomy):
                ...
                next_interaction = self.queue.get_head()
        if next_interaction is None and services.current_zone().is_zone_running and next_interaction is not None:
            if next_interaction.is_waiting_pickup_putdown:
                return WorkRequest()
            next_interaction.responsiveness_metrics_start()
            ...
            return WorkRequest(work_element=element, required_sims=required_sims, additional_resources=next_interaction.required_resources(), set_work_timestamp=next_interaction.set_work_timestamp, debug_name=str(next_interaction), can_cancel_fn=next_interaction.can_cancel_this_work)
        return WorkRequest()
    ...
    def on_outfit_changed(self, sim_info, category_and_index, old_category_and_index):
    ...
    @property
    def is_riding_horse(self):
        return self.posture.is_be_carried_posture and (self.posture_state.body_target.is_sim and self.posture_state.body_target.species == Species.HORSE)
