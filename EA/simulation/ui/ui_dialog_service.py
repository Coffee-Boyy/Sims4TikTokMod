...
class UiDialogService(Service):
    ...
    def dialog_pick_result_family_recipe(self, dialog_id:'int', picked_results=[], recipe_name:'str'=None, recipe_owner_name:'str'=None, ingredient_check:'bool'=None, prepped_ingredient_check:'bool'=None) -> 'bool':
        dialog = self._active_dialogs.get(dialog_id, None)
        if dialog is not None and dialog.pick_results_family_recipe(picked_results=picked_results, recipe_name=recipe_name, recipe_owner_name=recipe_owner_name, ingredient_check=ingredient_check, prepped_ingredient_check=prepped_ingredient_check):
            return True
        return False
