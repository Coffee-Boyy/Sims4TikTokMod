...
class FormationTypeFollow(FormationTypeBase):
    ...
    def find_good_location_for_slave(self, master_location):
        ...
        if fgl_tuning.slave_should_face_master:
            ...
            ignored_objects_ids = {self.master.id, self.slave.id}
            next_interaction = self.master.queue.peek_head()
            if next_interaction is not None and next_interaction.target is not None:
                ignored_objects_ids.add(next_interaction.target.id)
                logger.debug('Ignore object placement because of master interacting with it : {}', next_interaction.target)
            fgl_kwargs.update({'raytest_info': raytest_info, 'ignored_object_ids': ignored_objects_ids})
