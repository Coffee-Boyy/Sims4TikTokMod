...
class RetailZoneDirector(CustomerAndEmployeeZoneDirectorMixin, SchedulingZoneDirector):
    ...
    def create_situations_during_zone_spin_up(self):
        ...
        if is_owned_business and (self.business_manager.is_owner_household_active or services.current_zone().time_has_passed_in_world_since_zone_save() or services.current_zone().active_household_changed_between_save_and_load()):
            if self.business_manager.is_open:
                self._business_manager.start_already_opened_business()
            else:
                self.business_manager.try_open_npc_store()
