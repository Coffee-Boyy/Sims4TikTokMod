...
def transfer_retail_funds(amount:int, from_zone_id:int, to_zone_id:int, _connection=None):
    ...
    if from_business_manager is None:
        ...
        from_funds = from_business_manager.funds
        to_funds = to_business_manager.funds
        if from_business_manager.business_type == BusinessType.RENTAL_UNIT:
            household = services.household_manager().get(to_business_manager.owner_household_id)
            from_funds = household.funds
        if to_business_manager.business_type == BusinessType.RENTAL_UNIT:
            household = services.household_manager().get(from_business_manager.owner_household_id)
            to_funds = household.funds
        transfer_funds(amount, from_funds=from_funds, to_funds=to_funds)
    if from_business_manager is not None and from_business_manager.business_type != BusinessType.RENTAL_UNIT:
        from_business_manager.send_business_funds_update()
    if to_business_manager is not None and to_business_manager.business_type != BusinessType.RENTAL_UNIT:
