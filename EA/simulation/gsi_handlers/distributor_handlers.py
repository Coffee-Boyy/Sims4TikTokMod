...
def archive_operation(target_id, target_name, manager_id, message, payload_type, index, client):
    ...
    for (enum_name, enum_value) in message.DESCRIPTOR.enum_values_by_name.items():
        if enum_value.number == message.type:
            ...
            if message.type == DistributorOps_pb2.Operation.OBJECT_DELETE:
                message_type = 'OBJECT_DELETE'
            if '80' in str(message.data):
                message_type = 'FADE_OPACITY:IN'
            else:
                message_type = 'FADE_OPACITY:OUT'
            break
    blocker_entries = []
    tag_entries = []
