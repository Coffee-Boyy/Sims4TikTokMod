...
class TraitTracker(AffordanceCacheMixin, PreferenceTrackerMixin, GameplayObjectPreferenceTrackerMixin, SimInfoTracker):
    ...
    def _apply_trait(self, trait:'Trait', from_delayed_lod:'bool'=False) -> 'None':
        ...
        if sim is not None:
            ...
            TraitTracker._add_trait_animation_overlays_to_sim(sim, trait)
    ...
    def _remove_trait(self, trait:'Trait', skip_telemetry:'bool'=False) -> 'bool':
        ...
        if sim is not None:
            ...
            if trait.animation_overlays:
                if sim.has_component(types.ANIMATION_OVERLAY_COMPONENT):
                    for overlay in trait.animation_overlays:
                        sim.remove_overlay(overlay)
                else:
                    logger.warn('Cannot remove overlays from Sim for trait {} because Sim {} has no overlay component.', trait, sim)
    ...
    @staticmethod
    def _add_trait_animation_overlays_to_sim(sim:'Sim', trait:'Trait') -> 'None':
        if trait.animation_overlays:
            if sim.has_component(types.ANIMATION_OVERLAY_COMPONENT):
                for overlay in trait.animation_overlays:
                    sim.add_overlay(overlay)
            else:
                logger.warn('Trait {} has tuned Animation Overlays but receiving Sim {} has no overlay component.', trait, sim)
        ...
        for trait in tuple(self):
            if trait in self:
                ...
                if teleport_style_interaction is not None:
                    sim.add_teleport_style_interaction_to_inject(teleport_style_interaction)
                TraitTracker._add_trait_animation_overlays_to_sim(sim, trait)
                if sim.routing_context.ghost_route or trait.is_ghost_trait:
    ...
    def load(self, data, skip_load):
        ...
        for trait_instance_id in data.trait_ids:
            ...
            if not services.is_granted_or_non_account_reward_item(trait.guid64, AccountRewardType.TRAIT):
        ...
        if set(self._equipped_personality_traits) - set(self._ungranted_traits) or not self._sim_info.is_baby:
            possible_traits = [trait for trait in trait_manager.types.values() if trait.is_personality_trait and self.can_add_trait(trait) and services.is_granted_or_non_account_reward_item(trait.guid64, AccountRewardType.TRAIT)]
            if possible_traits:
        ...
        for trait in self:
            if not trait.display_overrides:
                ...
                for override in trait.display_overrides:
                    if filter_overrides:
                        if any([match.matches_on_any(match_age, match_occult_type, match_trait) for match in override.match_all]):
