...
class RentalUnitManager(BusinessRuleManagerMixin, BusinessManager):
    ...
    def cached_service_data_fixup(self, business_zone_id) -> 'None':
        business_service = services.business_service()
        cached_data = business_service.get_cached_rental_unit_data(business_zone_id)
        if cached_data is None:
            return
        self.due_rent = cached_data.due_rent
        self.dynamic_unit_rating = cached_data.dynamic_unit_rating
        self.active_rules = cached_data.active_rules
        self._has_tenant_ever_paid_rent = cached_data.has_tenant_ever_paid_rent
        self.house_description_id = cached_data.house_description_id
        self.max_rent = cached_data.max_rent
        self.overdue_rent = cached_data.overdue_rent
        self._rent = cached_data.rent
        self.signed_lease_length = cached_data.signed_lease_length
        self._open_time = cached_data._open_time
        self._total_open_hours = cached_data._total_open_hours
        self._is_open = True
        self._owner_household_id = None
        business_service.remove_cached_rental_unit_data(business_zone_id)
