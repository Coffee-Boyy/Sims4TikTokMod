...
class MultiUnitOwnershipService(Service):
    ...
    def _handle_sim_household_changed(self, sim_info:'SimInfo', event_type:'TestEvent', resolver:'Resolver') -> 'None':
        ...
        if sim_hh_id is None:
            return
        add = True
        if resolver.event_kwargs.get('sim_removed'):
            add = False
            sim_info_id = resolver.event_kwargs.get(event_testing.test_constants.TARGET_SIM_ID)
            sim_info = services.sim_info_manager().get(sim_info_id)
        if event_type == TestEvent.HouseholdChanged and sim_info.id == self._npc_property_owner_id:
