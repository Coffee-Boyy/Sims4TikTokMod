...
class Buff(RouteEventProviderMixin, InstancedSuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin, MixerActorMixin, MixerProviderMixin, TuningClassMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.BUFF)):
    ...
    @blueprintmethod
    def has_any_category(self, categories:'List[StatisticCategory]', categories_to_ignore:'List[StatisticCategory]') -> 'bool':
        buff_categories = self.get_commodity_categories()
        if buff_categories is None or categories and categories_to_ignore and any(category in categories_to_ignore for category in buff_categories):
            return False
        return any(category in categories for category in buff_categories)
    ...
    def on_add(self, from_load:'bool'=False, apply_buff_loot:'bool'=True, buff_source:'Union[Interaction, Loot, LootAction]'=None) -> 'None':
        ...
        if sim is not None:
            ...
            if not from_load:
                ...
                if self.buff_type.report_telemetry:
                    write_buff_telemetry(TELEMETRY_HOOK_ADD_BUFF, self, sim, buff_source=buff_source)
        if self.appearance_modifier is not None:
    ...
    def on_lod_increase(self, sim, old_lod, new_lod):
        ...
        for loot_actions in list(itertools.chain(self._loot_on_instance, self._loot_on_addition)):
            if isinstance(loot_actions, LootActions):
    ...
    def get_commodity_categories(self) -> 'Optional[List[StatisticCategory]]':
        owning_commodity = self._owning_commodity
        if owning_commodity is None:
            return
        else:
            categories = owning_commodity.get_categories()
            if categories:
                return categories
    ...
    def get_timeout_time_from_temporary_commodity_info(self, initial_value=DEFAULT) -> 'Tuple[int, int]':
        if not self.has_temporary_commodity:
            return NO_TIMEOUT
        time_now = services.time_service().sim_now
        duration = initial_value if initial_value is not DEFAULT else self._temporary_commodity_info.max_duration
        time_stamp = time_now + interval_in_sim_minutes(duration)
        return (time_stamp.absolute_ticks(), 1)
