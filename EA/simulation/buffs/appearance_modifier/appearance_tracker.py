        ...
        for (guid, attachment_ids) in self._active_sim_attachment_modifiers.items():
            if guid not in should_be_attached_guids:
                for attachment_id in attachment_ids:
                    ...
                    is_wing = attachment_id == AppearanceModifierDynamicAttachmentID.DYNAMIC_ATTACHMENT_FAIRY_WINGS
                    op = SetSimAttachment(attachment_id, SetSimAttachment.DETACH, is_wing=is_wing)
        ...
        for modifier_info in attachment_modifiers_to_apply:
            if modifier_info.guid not in already_attached_guids:
                ...
                if attachment_id is not None:
                    distributor = Distributor.instance()
                    attachment_target_id = 0
                    is_wing = self._appearance_target_obj is not None and is_wing_proxy_object(self._appearance_target_obj)
                    if not is_wing:
                        attachment_target_id = self._appearance_target_obj.id
                    op = SetSimAttachment(attachment_id, SetSimAttachment.ATTACH, attachment_target_id, is_wing)
                    distributor.add_op(self._sim_info, op)
