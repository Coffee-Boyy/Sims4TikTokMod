...
@sims4.commands.Command('restaurant.order_id_from_sim_id', command_type=sims4.commands.CommandType.Automation)
def get_order_id_from_sim_id(opt_sim:OptionalTargetParam=None, _connection=None):
    ordering_sim = get_optional_target(opt_sim, _connection)
    if ordering_sim is None:
        sims4.commands.automation_output('RestaurantGroupOrder; OrderID:0', _connection)
        return False
    zone_director = get_restaurant_zone_director()
    if zone_director is None:
        sims4.commands.automation_output('RestaurantGroupOrder; OrderID:0', _connection)
        return False
    group_order = zone_director.get_group_order(ordering_sim.id, create_if_not_found=False)
    if group_order is not None:
        sims4.commands.automation_output(f'RestaurantGroupOrder; OrderID:{group_order.order_id}', _connection)
        sims4.commands.output(f'RestaurantGroupOrder; OrderID:{group_order.order_id}', _connection)
        return True
    sims4.commands.automation_output('RestaurantGroupOrder; OrderID:0', _connection)
    return False

@sims4.commands.Command('restaurant.get_group_order_status', command_type=sims4.commands.CommandType.Automation)
def get_group_order_from_id(order_id:int, _connection=None):
    zone_director = get_restaurant_zone_director()
    if zone_director is None:
        sims4.commands.automation_output('RestaurantGroupOrder; Status:NoZoneDirector', _connection)
        return False
    group_order = zone_director.get_group_order_by_id(order_id)
    if group_order is not None:
        order_status = group_order.order_status.name
        sims4.commands.automation_output(f'RestaurantGroupOrder; Status:{order_status}', _connection)
        sims4.commands.output(f'RestaurantGroupOrder; Status:{order_status}', _connection)
        return True
    sims4.commands.automation_output('RestaurantGroupOrder; Status:NotFound', _connection)
    return False
