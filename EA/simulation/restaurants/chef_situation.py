...
class ChefSituation(BusinessEmployeeSituationMixin, StaffedObjectSituationMixin, SituationComplexCommon):
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if restaurant_zone_director is None:
            return
        if event == TestEvent.OnExitBuildBuy and self._has_any_cooking_object_been_destroyed():
            self._create_cooking_objects()
    ...
    def _has_any_cooking_object_been_destroyed(self) -> bool:
        chef_station = self.get_staffed_object()
        if chef_station is not None:
            return self._created_cutting_board is None or (self._created_cutting_board.id not in services.object_manager() or (self._created_pan is None or (self._created_pan.id not in services.object_manager() or (self._created_pot is None or self._created_pot.id not in services.object_manager()))))
        return False
