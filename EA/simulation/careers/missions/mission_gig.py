...
class MissionGig(HomeAssignmentGig):
    ...
    def get_additional_objectives(self):
        return [data.selected_objective for data in self._mission_objectives if data.is_valid]

    def get_active_objectives(self):
        return [data.selected_objective for data in self._mission_objectives if data.is_active]
    ...
    def set_up_gig(self):
        ...
        for objective_data in self.mission_objective_data:
            mission_objective = objective_data()
            mission_objective.initialize_mission_objective(self._owner, objectives_to_ignore=selected_objectives)
            self._mission_objectives.append(mission_objective)
            if mission_objective.is_invalid:
                pass
            elif not self.allow_duplicate_objectives:
                selected_objectives.append(mission_objective.selected_objective)
    ...
    def on_zone_load(self):
        super().on_zone_load()
        for objective_data in self._mission_objectives:
    ...
    def save_gig(self, gig_proto_buff):
        ...
        for mission_objective_data in self._mission_objectives:
    ...
    def load_gig(self, gig_proto_buff):
        ...
        for (i, save_objective_data) in enumerate(gig_proto_buff.mission_objective_data):
            ...
            self._mission_objectives.append(data)
    ...
    def complete_objective(self, objective):
        ...
        max_index = len(self._mission_objectives) - 1
        for (i, objective_data) in enumerate(self._mission_objectives):
            if objective_data.is_invalid:
                pass
            else:
                ...
                if selected_objective is objective:
                    ...
                    if i < max_index:
                        next_objective_data = self._mission_objectives[i + 1]
    ...
    def build_gig_msg(cls, inst, msg, sim, **kwargs):
        ...
        if inst is not None:
            msg.additional_objectives.extend([data.selected_objective.guid64 for data in inst._mission_objectives if data.is_valid])
...
class QuestGig(MissionGig):
            ...
            if objective_data.is_invalid:
                ...
                if selected_objective is objective:
                    ...
                    if i < max_index:
                        ...
                        if next_objective_data.requires_previous_objective_complete:
                            next_objective_data.activate_mission_objective(self._owner)
        active_tasks_list = [str(data.selected_objective.guid64) for data in self._mission_objectives if data.is_active]
    ...
    def build_gig_msg(cls, inst, msg, sim, **kwargs):
        ...
        if inst is not None:
            ...
            for data in inst._mission_objectives:
                if data.is_valid:
