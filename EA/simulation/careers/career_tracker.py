...
class CareerTracker(AffordanceCacheMixin, SimInfoTracker):
    ...
    def get_first_available_outfit_index(self) -> 'int':
        available_index = 0
        if not any(career.outfit_index == available_index for career in self.careers.values()):
    ...
    def add_career(self, new_career, show_confirmation_dialog=False, career_track=None, user_level_override=None, career_level_override=None, give_skipped_rewards=True, defer_rewards=False, post_quit_msg=True, schedule_shift_override=DEFAULT, show_join_msg=True, disallowed_reward_types=(), force_rewards_to_sim_info_inventory=False, defer_first_assignment=False, schedule_init_only=False, allow_outfit_generation=True, show_icon_override_picker=True, owner_id=None):
        ...
        if new_career.guid64 in self._careers:
            if owner_id is not None:
                self._careers[new_career.guid64].add_new_business(owner_id, 1)
            else:
    ...
    def remove_career(self, career_uid:'int', post_quit_msg:'bool'=True, update_ui:'bool'=True, test_event_origin:'TestEventCareersOrigin'=TestEventCareersOrigin.UNSPECIFIED, owner_id=None) -> 'None':
        if career_uid in self._careers:
            career = self._careers[career_uid]
            remove_career = True
            if owner_id:
                remove_career = career.remove_business(owner_id)
                if post_quit_msg and not remove_career:
                    career.send_career_message(career.career_messages.quit_career_notification)
            if remove_career:
                career.career_stop()
