...
class CareerBase(CoworkerMixin, CareerKnowledgeMixin, sims.sim_spawner_service.ISimSpawnerServiceCustomer):
    ...
    def should_skip_next_shift(self, check_if_can_go_now:'bool'=False, use_current_time:'bool'=False):
        if self.requested_day_off and self.requested_day_off_reason != CareerTimeOffReason.WORK_FROM_HOME:
            return True
        if use_current_time:
            start = services.time_service().sim_now
        else:
    ...
    def _start_work_from_home(self) -> 'None':
    ...
    def _on_early_warning_alarm_response(self, dialog:'UiDialog') -> 'None':
    ...
    def early_warning_callback(self):
        if not services.get_career_service().enabled:
            return
        if self.should_skip_next_shift(use_current_time=True) or self._should_automatically_use_pto():
            return
    ...
    def _get_drama_node_time_off_reason(self, end_time:'DateAndTime'=None):
        if end_time is None:
    ...
    def _start_work_callback(self, scheduler, alarm_data, extra_data):
        ...
        if self.requested_day_off_reason == CareerTimeOffReason.WORK_FROM_HOME:
            ...
            if not self.on_assignment:
                ...
                self._offered_assignment_ids = set(assignment.guid64 for assignment in assignments_to_offer)
                self._start_work_from_home()
        if self.should_skip_next_shift(check_if_can_go_now=True, use_current_time=True):
    ...
    def on_career_event_accepted(self, career_event:'CareerEvent', additional_sims:'Set[int]'=None, is_additional_sim:'bool'=False, start_situation_fn:'Callable[[int], int]'=None) -> 'None':
        self._start_career_event(career_event, additional_sims, is_additional_sim, start_situation_fn)
    ...
    def on_career_event_declined(self, career_event):
        ...
        if self.career_event_decline_option == CareerEventDeclineOptions.CAREER_RABBITHOLE:
            ...
            self.cancel_gig()
        elif self.career_event_decline_option == CareerEventDeclineOptions.WORK_ASSIGNMENTS:
            self._start_work_from_home()
        elif self.career_event_decline_option == CareerEventDeclineOptions.DO_NOTHING:
            pass

    def _start_career_event(self, career_event:'CareerEvent', additional_sims:'Set[int]', is_additional_sim:'bool', start_situation_fn:'Callable[[int], int]'=None) -> 'None':
        ...
        self._career_event_manager.start_top_career_event(start_situation_fn=start_situation_fn, additional_sims=additional_sims, is_additional_sim=is_additional_sim)
        self.resend_at_work_info()
    ...
    def attend_work(self, interaction=None, start_tones=True):
        ...
        if self._late_for_work_handle is not None:
            alarms.cancel_alarm(self._late_for_work_handle)
            self._late_for_work_handle = None
        self._show_start_work_notification()
        if start_tones:
            self.start_tones()
    ...
    def _apply_on_target(self, money_earned, pto_earned, gig=None):
        ...
        if self.currently_at_work:
            return EvaluationResult(Evaluation.ON_TARGET, self._get_daily_end_notification(), money_earned, pto_notification)
        if self.taking_day_off:
    ...
    def _handle_assignment_results(self):
        ...
        performance_mod = assignments_finished/assignments_total if assignments_total > 0 else 1
    ...
    def get_assignment_pay(self, assignments):
        ...
        hourly_pay = level_tuning.simoleons_per_hour
        if hourly_pay is 0 or career_track.active_assignment_amount is 0:
            return 0
    ...
    def populate_set_career_op_with_data_for_played_sims(self, career_op, gig=None):
        ...
        if self.career_category in [CareerCategory.Work, CareerCategory.AdultPartTime, CareerCategory.WorkFromHome]:
    ...
    def get_assignments_to_offer(self, just_accepted=False, forced_assignment=None):
        ...
        num_assignments = max(0, self.current_track_tuning.active_assignment_amount - 1)
