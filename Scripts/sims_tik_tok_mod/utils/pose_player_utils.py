...
import alarms
from date_and_time import create_time_span, REAL_MILLISECONDS_PER_SIM_SECOND, TimeSpan
...
class TikTokPosePlayerUtils:
    ...
    @staticmethod
    def play_pose_by_name(sim_info: SimInfo, pose_name: str, pose_duration: float = 1.1) -> bool:
                ...
                for si in sim.si_state:
                    if hasattr(si, 'pose_name') and si.pose_name == pose_name:
                        ...
                        if time_running > pose_duration:
                            ...
                            TikTokPosePlayerUtils._active_pose_alarms.pop(alarm_id, None).cancel()
            except Exception as e:
                LOG.error(f'Error stopping pose {pose_name}: {e}')

        one_second_time_span = TimeSpan(REAL_MILLISECONDS_PER_SIM_SECOND)
        alarm_handle = alarms.add_alarm(
            sim,
            one_second_time_span,
            _on_alarm,
            repeating=True,
            repeating_time_span=one_second_time_span
        )

        TikTokPosePlayerUtils._active_pose_alarms[alarm_id] = alarm_handle
